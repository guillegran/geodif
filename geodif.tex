...
\begin{defn}
  Consider two sets $X\subset \RR^n$ and $Y\subset \RR^m$. We say that a map $f:X\rightarrow Y$ is \emph{smooth} if there exist neighbourhoods $U\subset \RR^n$ and $V\subset \RR^m$ of $X$ and $Y$, respectively, and a smooth map $\tilde{f}:U\rightarrow V$ such that the following diagram commutes
  \begin{center}
    \begin{tikzcd}
     U \ar{r}{\tilde{f}}  & V  \\
     X \ar[hook]{u} \ar{r}{f} & Y. \ar[hook]{u}
    \end{tikzcd}
  \end{center}
...
\end{equation*}
determines the module structure. Giving a frame or a trivialization of $E$ over $U$ amounts to give a $C^\infty(U)$-module isomorphism $\Gamma(U,E)\cong C^\infty(U)^r$.
...
\chapter{Harmonic forms on compact manifolds}
