...
\thispagestyle{empty}
\begin{center}
\Large \bfseries A Master's course on Differential Geometry \par\vspace{2em}
...
\begin{defn}
  A \emph{derivation} or \emph{tangent vector} at $x\in M$ is an $\RR$-linear map $D:C^\infty_x\rightarrow \RR$ satisfying the \emph{Leibniz rule}
  \begin{equation*}
...
We remark that if $f\in C^\infty_x$ is represented by some pair $(U,f)$, with $f\equiv c$ a constant function, with $c\in \RR$, then, for any derivation $D\in T_xM$, we have $D(f)=0$. Indeed, since $D$ is assumed to be linear, we have that $D(f)=cD(1)$, and the Leibniz rule implies that $D(1)=D(1\cdot 1)=2D(1)$, so $D(1)=0$.
...
\section{Lie algebras and exponential map}
