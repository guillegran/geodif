...
\chapter{Un poco de análisis global}
\section{La ecuación de Poisson en variedades compactas}
\subsection{} Durante todo este capítulo, $M$ denota una variedad diferenciable compacta, orientada y sin borde. Fijamos, además, una métrica riemanniana $g$ en $M$, que en coordenadas $\bm{x}=(x^1,\dots,x^n)$ se escribe como
\begin{equation*}
g = g_{ij} dx^i dx^j.
\end{equation*}
...
\subsection{El producto $L^2$} La métrica riemanniana nos permite definir un producto escalar en el espacio de las funciones diferenciables $f:M\rightarrow \RR$, poniendo
\begin{equation*}
