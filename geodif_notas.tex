...
Fórmula pushforward y flujos. Flujos completos (p. ej. si $M$ compacta)
...
\begin{thm}[Hopf--Rinow]
  ...
  \end{enumerate}
  En ese caso, para cualesquiera dos puntos $x,y\in M$ existe una geodésica $\gamma:[0,d]\rightarrow M$ con $\gamma(0)=x$, $\gamma(d)=y$, para $d=d(x,y)$.
\end{thm}
...
\subsection{Holonomía} Tomemos ahora un lazo $\sigma:[0,1]\rightarrow M$, con $x=\sigma(0)=\sigma(1)$. Entonces el transporte paralelo determina una aplicación
\begin{equation*}
\Pi^A_\sigma: P_x \rightarrow P_x, \ p \mapsto \tilde{\sigma}_p(1).
\end{equation*}
Ahora, $p$ y $\Pi^A_\sigma(p)$ están en la misma fibra, de modo que existe un (único) $g\in G$ de modo que
\begin{equation*}
\Pi^A_\sigma(p) = p \cdot g.
\end{equation*}
Este elemento, que no depende de $p$, se denota por $\mathrm{Hol}^A_{x}(\sigma)$ y se denomina la \emph{holonomía} del lazo $\sigma$ determinada por $A$. Todo junto, esto determina una aplicación
\begin{equation*}
\mathrm{Hol}^A_x:\left\{ \text{Lazos en }M \right\} \rightarrow G,\ \sigma \mapsto \mathrm{Hol}^A_x(\sigma).
\end{equation*}
...
\begin{thm}
  Si $A$ es una conexión plana, entonces el transporte paralelo $\Pi_\gamma^A$ sólo depende de la clase de homotopía del camino $\gamma$. Por tanto, $\mathrm{Hol}^A_x$ desciende a un homomorfismo de grupos
  \begin{equation*}
\rho=\mathrm{Hol}^A_x: \pi_1(M,x) \rightarrow G,\ [\sigma] \mapsto \mathrm{Hol}^A_x(\sigma).
  \end{equation*}
  Además, fijado un punto $p_0\in P_x$, existe un isomorfismo canónico de $G$-fibrados principales
  \begin{equation*}
\Phi_{p_0}: \widetilde{M} \times^{\rho} G \rightarrow P,
  \end{equation*}
...
Definimos entonces
\begin{equation*}
\Psi_{p_0} :\widetilde{M} \times G \rightarrow P,\ ([\gamma], g) \mapsto \Pi^A_{\gamma}(p_0) \cdot g.
\end{equation*}
En efecto,
\begin{equation*}
\Psi_{p_0}([\gamma]\cdot [\sigma]^{-1}, \rho([\sigma]) \cdot g) = \Pi^A_{\gamma}(p_0)\mathrm{Hol}_{x}(\sigma)^{-1} \rho([\sigma]) g = \Psi_{p_0}([\gamma],g).
\end{equation*}
De modo que $\Psi_{p_0}$ desciende a una aplicación $\Phi_{p_0}:\widetilde{M} \times^\rho G \rightarrow P$, que es claramente un isomorfismo. En efecto, su inversa envía cada punto $p\in P$ a la clase del par $([\gamma],g)$, donde $[\gamma]$ es la clase de homotopía de un camino que une $x$ y $\pi(p)$ (que está bien definida salvo elementos de $\pi_1(M,x)$), y $g$ es el elemento tal que
$p = \Pi^A_\gamma(p_0) \cdot g$.
\end{proof}

\begin{rmk}
  \todo{terminar}
Explicar lo que pasa con la forma de Maurer-Cartan.
\end{rmk}
...
\end{equation*}
Por las propiedades de $\omega$, esta $1$-forma desciende a una $1$-forma $\fg$-valorada en $P_G$, que además determina una $G$-conexión en $P_G$, cuya $2$-forma de curvatura es exactamente $\Omega$. \todo{Comprobar esto bien}

\subsection{} Si $\tilde{M}\rightarrow M$ es un espacio recubridor y $(P,\omega)$ es una geometría de Cartan en $M$, obtenemos una geometría de Cartan $(\tilde{P},\tilde{\omega})$ inducida en $\tilde{M}$ simplemente poniendo $\tilde{P}=P \times_M \tilde{M}$ y $\tilde{\omega}= q^* \omega$, con $q:\tilde{P}\rightarrow P$ la aplicación natural.
...
\begin{prop}
  Si $(P,\omega)$ es una geometría de Cartan plana en una variedad conexa $M$, con recubridor universal $\tilde{M}$, entonces existe un isomorfismo de geometrías de Cartan
\begin{center}
  \begin{tikzcd}
   \tilde{P} \ar{r}{\psi} \ar{d} & G \ar{d} \\
   \tilde{M} \ar{r}{\varphi} & G/H.
  \end{tikzcd}
\end{center}
En particular, si $M$ es simplemente conexa, entonces $(P,\omega)$ es la geometría de Cartan trivial, y por tanto $M \cong G/H$.
\end{prop}

\begin{proof}
  La $1$-forma $\omega_G$ determina una conexión plana en $P_G$, y por tanto un isomorfismo $\Phi:\tilde{M}\times^\rho G \rightarrow P_G$, donde $\rho:\pi_1(M)\rightarrow G$ es la holonomía de $\omega_G$. Sobre $\tilde{M}$, la correspondiente $1$-forma $\tilde{\omega}_G$ determina un isomorfismo de $G$-fibrados principales $\Phi: \tilde{M}\times G \rightarrow \tilde{P}_G$. El $H$-fibrado principal $\tilde{P}\subset P_G$ determina una reducción del grupo de estructura de $\tilde{P}_G$ a $\tilde{P}$, es decir, una aplicación $G$-equivariante $\tau: \tilde{P}_G \rightarrow G/H$. Como $\tilde{P}_G$ es trivial, admite una sección global $s:\tilde{M}\rightarrow \tilde{P}_G$, y podemos definir \[\varphi=\tau \circ s: \tilde{M}\longrightarrow G/H.\]
  A su vez, como $\tilde{P}$ también es un $H$-fibrado trivial, tenemos un isomorfismo \[\psi:\tilde{P}\longrightarrow G/H \times H \cong G\] que cubre a $\varphi$.
\end{proof}
