...
\begin{thm}[Teorema de Frobenius. Versión clásica]
  Sea $U\times V \subset \RR^m \times \RR^n$ un subconjunto abierto, donde $U$ es un entorno de $0\in \RR^m$, y sean $F_i=(F_i^1,\dots,F_i^n):U\times V \rightarrow \RR^n$ funciones diferenciables, para $i=1,\dots,m$. Entonces, para cada $x\in V$ existe una (y sólo una) función diferenciable
  \begin{equation*}
...
\begin{cases}
  \alpha(0) = x, \\
  \frac{\partial \alpha}{\partial t^i} (t) = F_i(t,\alpha(t)), & \text{para todo } t\in W,
\end{cases}
  \end{equation*}
si y sólo si existe un entorno de $(0,x)\in U\times V$ en el que se cumple la siguiente \emph{condición de integrabilidad}
\begin{equation*}
