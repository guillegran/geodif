...
\begin{equation*}
II_x:=\mathrm{Hess}_x F: T_xS \times T_x S \rightarrow \RR.
\end{equation*}
...
Como la primera forma fundamental $I_x=g_x$ es no degenerada, ha de existir un operador $S_x:T_xS \rightarrow T_xS$, llamado el \emph{operador de forma}, de modo que
\begin{equation*}
II_x(v,w) = I_x(v,S_x(w)).
\end{equation*}
...
\end{pmatrix} \right|_x = - \mathrm{Hess}_x F,
  \end{equation*}
  que es la matriz asociada a $II_x$ en estas coordenadas euclidianas.
\end{proof}
...
\subsection{El tensor de Riemann} En secciones previas ya hemos introducido el \emph{tensor de curvatura de Riemann} $R\in \mathcal{T}^1_3(M)$ asociado a una variedad riemanniana $(M,g)$, que en coordenadas locales se expresa como
\begin{equation*}
