...
\cdots \rar{\delta_{i-1}} & H^i(A^\bullet) \rar & H^i(B^\bullet) \rar & H^i(C^\bullet) \rar{\delta_{i}} & H^{i+1}(A^\bullet) \rar & \cdots.
            \end{tikzcd}
          \end{center}
    \item Sólo necesitaremos considerar complejos $A^\bullet$ que sean \emph{inferiormente acotados}: esto quiere decir que existe un $n \in \mathbb{Z}$ tal que $A^i=0$ para todo $i<n$. Esto nos determina una subcategoría abeliana $\Ch^+(\sfA)$ de complejos de cocadenas inferiormente acotados.
  \end{enumerate}
\end{rmk}

\begin{defn}
  Un morfismo de complejos de cocadenas $f:A^\bullet \rightarrow B^\bullet$ es un \emph{cuasi-isomorfismo} si el morfismo inducido $f_*:H^i(A^\bullet)\rightarrow H^i(B^\bullet)$ es un isomorfismo para todo $i$. Se define la \emph{categoría derivada (inferiormente acotada) de $\sfA$} como la categoría $\DD^+(\sfA)$ con la siguiente propiedad universal: existe un único functor $\cI:\Ch^+(\sfA) \rightarrow \DD^+(\sfA)$ tal que
  \begin{enumerate}
    \item $\cI$ envía cuasi-isomorfismos en isomorfismos,
    \item para cualquier otra categoría $\sfC$ y cualquier otro funtor $F:\Ch^+(\sfA)\rightarrow \sfC$ que envíe cuasi-isomorfismos en isomorfismos, se tiene que $F$ factoriza por $\cI$.
  \end{enumerate}
\end{defn}
...
\subsection{} La homotopía determina una relación en los morfismos de $\Ch^+(\sfA)$. Esto nos permite cocientar por esta relación, y definir la categoría $\KK^+(\sfA)$ cuyos objetos son los mismos que los de $\Ch^+(\sfA)$, pero ahora
\begin{equation*}
\Hom_{\KK^+(\sfA)}(A^\bullet,B^\bullet) = \Hom_{\Ch^+(\sfA)}(A^\bullet,B^\bullet)\ /\text{ homotopía}.
\end{equation*}
Se ve inmediatamente de la definición de homotopía que dos morfismos homotópicos definen el mismo morfismo en las cohomologías, de modo que los funtores $H^i:\Ch_+(\sfA)\rightarrow \sfA$ descienden a funtores $H^i:\KK_+(\sfA)\rightarrow \sfA$.
Una \emph{equivalencia de homotopía} es un morfismo en $\Ch^+(\sfA)$ que determina un isomorfismo en $\KK^+(\sfA)$. Decimos que dos complejos de cocadenas son \emph{homotópicamente equivalentes} si son isomorfos en $\KK^+(\sfA)$.

\subsection{} Un complejo de cadenas $A^\bullet$ se dice \emph{contractible} si existe una homotopía entre la identidad $\id_{A^\bullet}$  y $0$. En otras palabras, $A^\bullet$ es contractible si existe un morfismo de complejos $h:A^\bullet \rightarrow A^{\bullet-1}$ tal que
\begin{equation*}
...
\subsection{} Nótese que a priori puede haber complejos que no homotópicamente equivalentes, pero que sí sean cuasi-isomorfos, de modo que la categoría derivada no los distingue. Si queremos hallar unos representantes adecuados, debemos buscar complejos que sean isomorfos en $\KK^+(\sfA)$ si y sólo si son cuasi-isomorfos. Para ello introducimos los objetos inyectivos.

\begin{defn}
Un objeto $I$ en una categoría es \emph{inyectivo} si, para todo monomorfismo $i:X\rightarrow Y$ y para todo morfismo $f:X\rightarrow I$, existe un morfismo $g:Y \rightarrow I$ tal que $g\circ i = f$.
\end{defn}

\begin{prop}
Sea $I^\bullet$ es un complejo de cocadenas acotado inferiormente de objetos inyectivos. Si $I^\bullet$ es cuasi-isomorfo a $0$, entonces es contractible.
\end{prop}
