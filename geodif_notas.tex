...
\subsection{}
Podemos usar el resultado anterior para obtener generadores del anillo $\RR[\fg]^G$, donde $G$ es uno de los grupos clásicos. Vamos a ver en particular el caso de $G=\mathrm{SO}_n(\RR)$; el resto se obtienen de forma similar, y pueden encontrarse en \todo{CITAR:Kobayashi-NomizuVol2}.

Comenzamos considerando un elemento $A\in \mathfrak{so}_n(\RR)$ (es decir, una matriz $n\times n$ antisimétrica) y tomando su polinomio característico
\begin{equation*}
p_A(T) = \det(T \mathbb{I}_n - A) = T^n + f_1(A) T^{n-2} + f_2(A) T^{n-4} + \cdots + f_m(A).
\end{equation*}
Esto nos determina unas funciones $f_1,\dots,f_m \in \RR[\mathfrak{so}_n(\RR)]$. Como el polinomio característico es invariante por conjugación, estas funciones pertencen a $\RR[\mathfrak{so}_n(\RR)]^{\mathrm{SO}_n(\RR)}$.  Nótese que, como $(T \mathbb{I}_n - A)^T = T \mathbb{I}_n + A$, tenemos que $p_A(-T)=(-1)^n p_A(T)$, con lo cual el coeficiente que acompaña a cada $T^{n-i}$, con $i$ impar, se anula.

En el caso en que $n=2m$ es par, existe otra función $\mathrm{Pf}\in \RR[\mathfrak{so}_n(\RR)]$, denominada el \emph{pfaffiano}, que cumple que
\begin{equation*}
...
\end{equation*}
En particular, para $g\in \mathrm{GL}_{2m}(\RR)$, se tiene que $\mathrm{Pf}(gAg^T)=\det(g)\mathrm{Pf}(A)$, luego $\mathrm{Pf}$ pertenece al anillo de invariantes $\RR[\mathfrak{so}_n(\RR)]^{\mathrm{SO}_n(\RR)}$.

\begin{thm}
  Si $n=2m+1$ es impar, entonces
  \begin{equation*}
\RR[\mathfrak{so}_n(\RR)]^{\mathrm{SO}_n(\RR)} = \RR[f_1,\dots,f_m].
  \end{equation*}
  Si $n=2m$ es par, entonces
  \begin{equation*}
\RR[\mathfrak{so}_n(\RR)]^{\mathrm{SO}_n(\RR)} = \RR[f_1,\dots,f_{m-1},\mathrm{Pf}].
  \end{equation*}
\end{thm}

\begin{proof}
Definimos $T\subset \mathrm{SO}_n(\RR)$ como el subgrupo de Lie conexo cuya álgebra de Lie $\mathfrak{t}$ está formada por las matrices de la forma
...
\end{pmatrix},
\end{equation*}
si $n=2m+1$ es impar. Toda matriz antisimétrica puede conjugarse por un elemento de $\mathrm{SO}_n(\RR)$ a una matriz de la forma anterior. Por tanto, se cumplen las hipótesis del teorema previo y tenemos que la restricción induce un isomorfismo
\begin{equation*}
\RR[\mathfrak{so}_n(\RR)]^{\mathrm{SO}_n(\RR)} \rightarrow \RR[\mathfrak{t}]^N,
\end{equation*}
para
\begin{equation*}
N = \left\{ A \in \mathrm{SO}_n(\RR): A \mathfrak{t} A^{-1} \subset \mathfrak{t} \right\}.
\end{equation*}
...
\end{pmatrix}.
\end{equation*}
También, para cada $i\in \left\{ 1,\dots,m \right\}$ podemos encontrar una matriz $B \in \mathrm{O}_n(\RR)$ con $B\mathfrak{t} B^{-1}\subset \mathfrak{t}$ tal que
\begin{equation*}
...
\end{pmatrix},
\end{equation*}
que cumple las misma propiedades, y que además es un elemento de $\mathrm{SO}_n(\RR)$, y por tanto de $N$. Esto prueba una igualdad
\begin{equation*}
\RR[\mathfrak{t}]^N = \RR[(x_1)^2,\dots,(x_m)^2]^{\mathfrak{S}_m}=\RR[f_1,\dots,f_m],
\end{equation*}
para $n=2m+1$, y por tanto un isomorfismo
\begin{equation*}
\RR[\mathfrak{so}_{2m+1}(\RR)]^{\mathrm{SO}_{2m+1}(\RR)} \cong \RR[f_1,\dots,f_m].
\end{equation*}
...
\begin{equation*}
\RR[\mathfrak{so}_{2m}(\RR)]^{\mathrm{SO}_{2m}(\RR)} \cong \RR[f_1,\dots,f_{m-1},\mathrm{Pf}],
\end{equation*}
como queríamos probar.
\end{proof}
...
& +k \int_0^1 f(\alpha \wedge F_t \wedge \overset{k-1}{\cdots} \wedge \pi_{H_{A_t}}^*dF_t) dt = k \int_0^1 f\left(\frac{dF_t}{dt} \wedge F_t \wedge \overset{k-1}{\cdots} \wedge F_t\right) dt  \\
  &= \int_0^1 \frac{d}{dt} f(F_t) dt = f(F_0) - f(F_1),
\end{align*}
como queríamos probar.
\end{proof}
...
\section{La clase de Euler y el teorema de Gauss--Bonnet}
\subsection{La clase de Euler} Dado un $\SO_{2m}(\RR)$-fibrado principal $P\rightarrow M$, se define su \emph{clase de Euler} $e(P)\in H^{2m}(M,\RR)$ como la imagen del pfaffiano $\mathrm{Pf} \in \RR[\mathfrak{so}_{2m}(\RR)]^{\SO_{2m}(\RR)}$ por el homomorfismo de Chern--Weil,
\begin{equation*}
e(P) = w_P(\mathrm{Pf}) \in H^{2m}(M,\RR).
\end{equation*}
Es decir,
\begin{equation*}
e(P) = \overline{\mathrm{Pf}}(F_A),
\end{equation*}
para cualquier conexión $A$ en $P$.

La clase de Euler cumple las siguientes propiedades, que la caracterizan axiomáticamente.

\begin{thm} \leavevmode
\begin{enumerate}
  \item  (Naturalidad). Dados un $\SO_{2m}(\RR)$-fibrado principal $P\rightarrow M$ y una aplicación diferenciable $f:M'\rightarrow M$, se tiene que
        \begin{equation*}
e(f^* P ) = f^*(e(P))
        \end{equation*}
  \item (Normalización). Si $L$ es un $S^1$-fibrado principal, visto como un $\SO_2(\RR)$ fibrado principal por medio del isomorfismo natural $S^1\cong \SO_2(\RR)$, entonces la clase de Euler de $L$ coincide con su clase de Chern \todo{REF},
        \begin{equation*}
e(L)= c_1(L).
        \end{equation*}
  \item (Fórmula de la suma de Whitney). Sean $L_1,\dots,L_m$ $S^1$-fibrados principales, y sean $E_1,\dots,E_m$ los correspondientes fibrados vectoriales de rango $2$ asociados a ellos por la representación natural
        \begin{equation*}
S^1 \cong \SO_2(\RR) \hookrightarrow \GL(\RR^2).
        \end{equation*}
        Sea $P$ el $\SO_{2m}(\RR)$-fibrado principal obtenido como el fibrado de referencias del fibrado vectorial $E=E_1\oplus \cdots \oplus E_m$ compatibles con las estructuras $\SO_2(\RR)$ de cada $E_i$. Se tiene que
        \begin{equation*}
e(P) = e(L_1)\cdots e(L_m).
        \end{equation*}
\end{enumerate}
\end{thm}
...
\begin{thm}[Teorema de Chern--Gauss--Bonnet]
  Sea $(M,g)$ una variedad riemanniana orientada de dimensión par $n=2m$ y sea $P=\mathcal{B}_{\SO}(M)$ su fibrado de referencias ortogonales compatibles con la orientación, que es un $\SO_{2m}(\RR)$-fibrado principal. Se tiene que
  \begin{equation*}
\int_{M} e(P) = (2\pi)^k \chi(M),
  \end{equation*}
donde $\chi(M)$ es la característica de Euler de $M$. En particular, si $R$ es la curvatura de $M$, se tiene que
  \begin{equation*}
\int_{M} \overline{\mathrm{Pf}}(R) = (2\pi)^k \chi(M).
  \end{equation*}
\end{thm}
