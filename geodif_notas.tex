...
\subsection{} Durante todo este capítulo, $M$ denota una variedad diferenciable conexa, compacta, orientada y sin borde. Fijamos, además, una métrica riemanniana $g$ en $M$, que en coordenadas $\bm{x}=(x^1,\dots,x^n)$ se escribe como
\begin{equation*}
g = g_{ij} dx^i dx^j.
\end{equation*}
...
\subsection{} Nótese ahora que una función $\phi \in C_0^\infty(M)$ satisface la ecuación $\Delta_g \phi = \rho$ si y sólo si, para toda función $f\in C_0^\infty(M)$ se cumple que
\begin{equation*}
...
\section{Regularidad de las soluciones}
