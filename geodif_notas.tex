...
En el caso en que $n=2m$ es par, existe otra función $p\in \RR[\mathfrak{so}_n(\RR)]$, denominada el \emph{pfaffiano}, que cumple que
\begin{equation*}
p(A)^2 = f_m(A) = \det(A)
\end{equation*}
y se define como sigue. Si denotamos $A=(a_{ij})_{i,j}$, entonces
\begin{equation*}
p(A) = \frac{1}{2^m m!} \sum_{\sigma \in \mathfrak{S}_{2m}} (-1)^\sigma \prod_{i=1}^m a_{\sigma(2i-1)\sigma(2i)}.
\end{equation*}
...
entonces
\begin{equation*}
\omega_A^{m} = p(A) dx^1 \wedge \cdots dx^{2m}.
\end{equation*}
En particular, para $g\in \mathrm{GL}_{2m}(\RR)$, se tiene que $p(gAg^T)=\det(g)p(A)$, luego $p$ pertenece al anillo de invariantes $\RR[\mathfrak{so}_n(\RR)]^{\mathrm{SO}_n(\RR)}$.
...
\RR[\mathfrak{so}_n(\RR)]^{\mathrm{SO}_n(\RR)} = \RR[f_1,\dots,f_m].
  ...
  \begin{equation*}
\RR[\mathfrak{so}_n(\RR)]^{\mathrm{SO}_n(\RR)} = \RR[f_1,\dots,f_{m-1},p].
  \end{equation*}
\end{thm}
...
f = g + x_1\cdots x_m h,
\end{equation*}
para $g,h \in \RR[(x_1)^2,\dots,(x_m)^2]$. Además, como $f$ es simétrica en las variables $x_1,\dots,x_m$, tenemos que $g,h\in \RR[(x_1)^2,\dots,(x_m)^2]^{\mathfrak{S}_m}=\RR[f_1,\dots,f_m]$. Finalmente, como $p$ restringido a $\mathfrak{t}$ es igual al polinomio $x_1\cdots x_m$ y $p^2=f_m$, concluimos que
\begin{equation*}
\RR[\ft]^N = \RR[f_1,\dots,f_{m-1},p],
\end{equation*}
y por tanto tenemos un isomorfismo
\begin{equation*}
\RR[\mathfrak{so}_{2m}(\RR)]^{\mathrm{SO}_{2m}(\RR)} \cong \RR[f_1,\dots,f_{m-1},p],
\end{equation*}
como queríamos probar.
\end{proof}
...
\section{Clases de Euler y el teorema de Gauss--Bonnet}
aa
%a
%
%Aquí Poincaré--Hopf
