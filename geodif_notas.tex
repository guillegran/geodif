...
\begin{prop}
  Si $(P,\omega)$ es una geometría de Cartan plana en una variedad conexa $M$, con recubridor universal $\widetilde{M}$, entonces existe una aplicación diferenciable $\psi:\widetilde{P}\rightarrow G$, que induce una aplicación $\varphi:\widetilde{M}\rightarrow G/H$ a través del diagrama
\begin{center}
  \begin{tikzcd}
...
%y de modo que $(\psi,\varphi)$ determina un isomorfismo local de geometrías de Cartan entre $(P,\omega)$ y la geometría de Cartan trivial; es decir, para todo $x\in M$ existe un entorno $U\subset M$ de $x$ tal que $(\varphi|_U)^* \omega_G = \omega|_U$.
\end{prop}

\begin{proof}
  La $1$-forma $\omega_G$ define una conexión plana en $P_G$, y por tanto un isomorfismo $\Phi:\widetilde{M}\times^\rho G \rightarrow P_G$, donde $\rho:\pi_1(M)\rightarrow G$ es la holonomía de $\omega_G$. Sobre $\widetilde{M}$, la correspondiente $1$-forma $\widetilde{\omega}_G$ determina un isomorfismo de $G$-fibrados principales $\Phi: \widetilde{M}\times G \rightarrow \widetilde{P}_G$. El $H$-fibrado principal $\widetilde{P}\subset \widetilde{P}_G$ da una reducción del grupo de estructura de $\widetilde{P}_G$ a $H$, es decir, una aplicación $G$-equivariante $\tau: \widetilde{P}_G \rightarrow G/H$. Como $\widetilde{P}_G$ es trivial, admite una sección global $s:\widetilde{M}\rightarrow \widetilde{P}_G$, y podemos definir \[\varphi=\tau \circ s: \widetilde{M}\longrightarrow G/H.\]
  A su vez, como $\widetilde{P}$ también es un $H$-fibrado trivial, tenemos una aplicación \[\psi\cong (\varphi,\id_H):\widetilde{P}\cong \widetilde{M} \times H\longrightarrow G/H \times H \cong G\] que cubre a $\varphi$.

  \todo{TERMINAR. Maurer-Cartan}
\end{proof}
