...
\begin{defn}
  Sea $M$ una variedad diferenciable. Para cada $k\in \Z$, se define el \emph{$k$-ésimo grupo de cohomología de $M$} (con coeficientes en $\RR$) como el grupo de cohomología de haces
  \begin{equation*}
H^k(M,\RR) := H^k(M,\underline{\RR}_M),
  \end{equation*}
donde $\underline{\RR}_M$ es el haz constante $\RR$ en $M$.
\end{defn}
