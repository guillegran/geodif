...
\begin{equation*}
f(x',\dots,x') = 0, \ \text{para todo } x' \in \fg'
\end{equation*}
...
\begin{equation*}
p_A(T) = \det(\lambda \mathbb{I}_n - A) = T^n + f_1(A) T^{n-2} + f_2(A) T^{n-4} + \cdots + f_m(A).
\end{equation*}
Esto nos determina unas funciones $f_1,\dots,f_m \in \RR[\mathfrak{so}_n]$. Como el polinomio característico es invariante por conjugación, estas funciones pertencen a $\RR[\mathfrak{so}_n(\RR)]^{\mathrm{SO}_n(\RR)}$.  Nótese que, como $(\lambda \mathbb{I}_n - A)^T = \lambda \mathbb{I}_n + A$, tenemos que $p_A(-T)=(-1)^n p_A(T)$, con lo cual el coeficiente que acompaña a cada $T^{n-i}$, con $i$ impar, se anula.
...
\end{equation*}
En particular, para $g\in \mathrm{GL}_{2n}(\RR)$, se tiene que $p(gAg^T)=\det(g)p(A)$, luego $p$ pertenece al anillo de invariantes $\RR[\mathfrak{so}_n(\RR)]^{\mathrm{SO}_n(\RR)}$.
...
\end{pmatrix},
\end{equation*}
si $n=2m+1$ es impar. Toda matriz antisimétrica puede conjugarse por un elemento de $\mathfrak{SO}_n(\RR)$ a una matriz de la forma anterior. Por tanto, se cumplen las hipótesis del teorema previo y tenemos que la restricción induce un isomorfismo
\begin{equation*}
...
\begin{equation*}
A [x_1,\dots,x_i,\dots,x_j,\dots,x_m] A^{-1} = [x_1,\dots,x_i,\dots,x_j,\dots,x_m].
\end{equation*}
