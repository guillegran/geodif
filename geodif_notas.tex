...
\subsection{El haz de estructura}
  ...
  \begin{equation*}
C^\infty(M):= \left\{ f:M\rightarrow \RR : f\text{ es diferenciable} \right\}.
  \end{equation*}
  Más generalmente, para cada subconjunto abierto $U\subset M$, podemos considerar el anillo
  \begin{equation*}
C^\infty(U):= \left\{ f:U\rightarrow \RR : f\text{ es diferenciable} \right\}.
  \end{equation*}
  Si $U\subset V\subset M$ son subconjuntos abiertos, tenemos una aplicación de restricción $C^\infty(V)\rightarrow C^\infty(U)$, $f\mapsto f|_U$. Se dice entonces que la asignación $U\mapsto C^\infty(U)$ determina un \emph{funtor contravariante} de la categoría de los abiertos de $M$ con sus inclusiones a la categoría de $\RR$-álgebras\footnote{Más sobre teoría de categorías en el capítulo \todo{REF}.}. Dicho funtor se denota por $C^\infty_M$ y se denomina el \emph{haz de estructura} de $M$.

  Es posible recuperar la estructura diferenciable del haz de estructura. En efecto, basta decir que un sistema de coordenadas $(U,\bm{x})$ está en el correspondiente atlas maximal si y sólo si, para toda función diferenciable $f: \RR^n \rightarrow \RR$, la composición $f|_{\bm{x}(U)} \circ \bm{x}:U\rightarrow \RR$ pertenece al anillo $C^\infty(U)$.
...
\begin{equation*}
(\bm{x} \circ \gamma_1)' (0) = (\bm{x} \circ \gamma_2)'(0).
\end{equation*}
...
\subsection{Modelo 2: Derivaciones} Un \emph{germen de función diferenciable en $x$} es un par $(U,f)$ formado por un entorno abierto $U\subset M$ de $x$ y una función diferenciable $f:U\rightarrow \RR$. Dos gérmenes $(U_1,f_1)$ y $(U_2,f_2)$ son equivalentes si existe un subconjunto abierto $V\subset U_1\cap U_2$ de modo que $f_1|_V=f_2|_V$. Los gérmenes de funciones diferenciables en $x$ forman un anillo, que denotamos por $C^\infty_{M,x}$. Dicho anillo está equipado con una aplicación $\mathrm{ev}_{x}:C^\infty_{M,x}\rightarrow \RR$ dada por la evaluación en $x$
\begin{equation*}
\mathrm{ev}_x([U,f]) = f(x).
\end{equation*}
Una \emph{derivación} en $x$ es una aplicación $\RR$-lineal $X_x: C^\infty_{M,x} \rightarrow \RR$ que cumple la \emph{regla de Leibniz}
\begin{equation*}
...
\end{equation*}
La notación tiene sentido ya que, para cualquier $f\in C^\infty_{M,x}$, se tiene que
\begin{equation*}
...
\begin{pmatrix}
  ...
  \vdots \\
  Y^n
\end{pmatrix} =  J_{\bm{x}(x)}(\bm{y} \circ f \circ \bm{x}^{-1})
\begin{pmatrix}
  X^1 \\
  \vdots \\
  X^n
...
\begin{equation*}
f: \RR^{n+1} \rightarrow \RR,\ (x^1,\dots,x^{n+1}) \mapsto (x^1)^2 + \cdots + (x^{n+1})^2 - 1.
\end{equation*}
...
\end{pmatrix},
\end{equation*}
que tiene rango $1$ para todo $(x^1,\dots,x^{n+1}) \in S^n$.
...
\subsection{Ejemplo: El fibrado de Möbius} El ejemplo más simple de un fibrado vectorial no trivial está dado por la bien conocida construcción de Möbius. Definimos
\begin{equation*}
E = \frac{[0,1]\times \RR}{(0,t)\sim (1,-t)}
\end{equation*}
y
\begin{equation*}
\pi:E \rightarrow S^1,\ [s,t] \mapsto e^{2\pi i s}.
\end{equation*}
Si el fibrado fuera trivial existiría una trivialización global
\begin{equation*}
\varphi: E \rightarrow S^1 \times \RR.
\end{equation*}
Consideramos entonces la aplicación
\begin{equation*}
s: S^1\rightarrow S^1\times \RR,\ x \mapsto (x,1)
\end{equation*}
y la composición $s'=\varphi^{-1}\circ s: S^1 \rightarrow E$. Esta aplicación levanta a una aplicación $f:[0,1]\rightarrow [0,1] \times \RR$. Como $\varphi$ es un isomorfismo lineal en cada fibra, para cada $x\in S^1$, el punto $s'(x)\in E_x$ es un elemento de $E_x$ distinto de $0$. Por tanto, $f$ ha de satisfacer las condiciones
\begin{equation*}
\begin{cases}
f(t)\neq 0 & \text{para todo } t \in [0,1], \\
f(1) = -f(0).
\end{cases}
\end{equation*}
Esto contradice el teorema de Bolzano.

Otra forma equivalente de definir el fibrado de Möbius es como el \emph{fibrado tautológico} $E\rightarrow \mathbb{RP}^1$, donde identificamos la circunferencia $S^1$ con la recta proyectiva real $\mathbb{RP}^1$. El espacio total de este fibrado es
\begin{equation*}
E = \left\{ (l,v) \in \mathbb{RP}^1 \times \RR^2 : v\in l \right\},
\end{equation*}
y la proyección $E\rightarrow \mathbb{RP}^1$ es la restricción de la proyección natural $\mathrm{pr}_1:\mathbb{RP}^1 \times \RR^2\rightarrow \mathbb{RP}^1$. De la misma forma se puede definir el fibrado tautológico $E\rightarrow \mathbb{RP}^n$, de rango $n$.
...
\subsection{Fibrados y $C^\infty_M$-módulos} Dado un fibrado vectorial $E\rightarrow M$, para cualquier abierto $U\subset M$, el espacio de secciones $\Gamma(U,E)$ es un módulo sobre el anillo $C^\infty(U)$ de las funciones diferenciables $U\rightarrow \RR$. En efecto, dada cualquier sección $s:U\rightarrow E$, y cualquier función $f:U\rightarrow \RR$, el producto $fs:U\rightarrow E$ sigue siendo una sección de $E$ sobre $U$. Globalmente, decimos que el \emph{haz de secciones} $\Gamma_E:U\mapsto \Gamma(U,E)$ es un $C^\infty_M$-módulo. En general, $\Gamma(U,E)$ no es un $C^\infty(U)$-módulo libre, pero, si $U$ es tal que $E|_U$ es isomorfo al fibrado trivial $U\times \RR^r$, entonces este isomorfismo induce un isomorfismo $\Gamma(U,E)\cong C^\infty(U)^r$. Este isomorfismo determina una base $e_1,\dots,e_r$ de $\Gamma(U,E)$ como $C^\infty(U)$-módulo, que se denomina una \emph{referencia} de $E$. En particular, si $E=TM$, una referencia de $TM$ sobre un abierto $U\subset M$ se denomina una \emph{referencia móvil} (o, simplemente, una \emph{referencia}) de $M$. Como, por definición, los fibrados son localmente triviales, el haz de secciones $\Gamma_E$ es un haz \emph{localmente libre}. De hecho, esto da una equivalencia de categorías entre los haces localmente libres de $C^\infty_M$-módulos y los fibrados vectoriales sobre $M$.
...
\subsection{Campos, formas, métricas y tensores} Sea $M$ una variedad diferenciable. Un \emph{campo tensorial} (o, simplemente un \emph{tensor}) \emph{$r$ veces contravariante y $s$ veces covariante} (equivalentemente, \emph{de tipo $(r,s)$}), es una sección del fibrado vectorial
\begin{equation*}
T^{r}_s (M) = (TM)^{\otimes r} \otimes (T^*M)^{\otimes s}.
\end{equation*}
...
Un tensor de tipo $(1,0)$ es simplemente una sección de $TM$, es decir, un \emph{campo vectorial}. El espacio de los campos vectoriales se denota por
\begin{equation*}
\fX(M) = \mathcal{T}^1_0(M) = \Gamma(M,TM).
\end{equation*}

Un tensor de tipo $(0,2)$ determina una forma bilineal en cada espacio tangente $T_xM$, $x\in M$. En particular, si el tensor es simétrico y la forma es no degenerada en cada punto, el tensor se denomina una \emph{métrica} (riemanniana).

Un tensor antisimétrico de tipo $(0,k)$ se denomina una \emph{forma diferencial de grado $k$} o \emph{$k$-forma diferencial}. El espacio de las $k$-formas diferenciales se denota como
\begin{equation*}
...
\subsection{Formas con soporte compacto} Una forma diferencial $\omega \in \Omega(M)$ tiene \emph{soporte compacto} si existe un subconjunto compacto $K\subset M$ de forma que $\omega_x = 0$ para todo $x\not \in K$. Nótese que si $M$ es compacta, entonces podemos tomar $K=M$, y todas las formas tienen soporte compacto. Denotamos por $\Omega_c(M)\subset \Omega(M)$ (resp. $\Omega_c^k(M)\subset \Omega^k(M)$) al subespacio formado por las formas diferenciales (resp. de grado $k$) con soporte compacto.
...
\subsection{Cohomología de Eilenberg--Steenrod} Los grupos de cohomología (relativa, con coeficientes en $\RR$) se pueden definir de manera axiomática como una colección de funtores contravariantes $H^k$, para $k\in \Z$, de la categoría de pares $(X,A)$, donde $X$ es un espacio topológico y $A\subset X$ un subconjunto, a la categoría de grupos abelianos que cumplen las propiedades que enunciamos a continuación. De ahora en adelante, si $(X,A)$ es un par, denotamos su \emph{$k$-ésimo grupo de cohomología relativa} como
\begin{equation*}
H^k(X,A;\RR),
\end{equation*}
y, si $f$ es una aplicación continua entre pares, denotamos $f^*=H^k(f)$ para cualquier $k$. Denotamos el \emph{$k$-ésimo grupo de cohomología} de $X$ como
\begin{equation*}
H^k(X,\RR) = H^k(X,\varnothing;\RR).
\end{equation*}
...
\begin{thm}
Los grupos de cohomología relativa existen y son únicos (salvo isomorfismo natural).
\end{thm}

\begin{rmk}
Hay varias formas de construir los grupos $H^k(X,A;\RR)$. La manera clásica es la \emph{cohomología singular}, en términos de los grupos de cadenas singulares (combinaciones lineales de cubos en $X$). Otra forma típica, que funciona en espacios paracompactos es la cohomología de \v{C}ech, en términos de recubrimientos por abiertos. La forma más directa es usando la teoría de haces: el grupo $H^k(X,A;\RR)$ es el grupo de cohomología de haces $H^k(j_{!}\underline{\RR}_U)$, donde $U=X\setminus A$ es el complemento de $A$, $j_!$ denota la extensión por $0$, y $\underline{\RR}_U$ es el haz constante $\RR$ en $U$.
\end{rmk}

\begin{rmk}
Como $\RR^n$ es homotópicamente equivalente a un punto, sus grupos de cohomología son
  \begin{equation*}
    H^k(\RR^n,\RR) =
    \begin{cases}
\RR & \text{ si } k= 0,\\
...
\end{cases}
  \end{equation*}
  Este hecho se conoce como el \emph{lema de Poincaré}.
\end{rmk}
...
\begin{rmk}
  Esta es una versión algo abstracta del teorema de de Rham. Una afirmación más concreta es que la aplicación dada por la integral
  \begin{equation*}
C^k(M) \times \Omega^k(M) \rightarrow \RR,\ (\sigma,\omega) \mapsto \int_{[0,1]^k} \sigma^* \omega
  \end{equation*}
emparejando $k$-cadenas singulares con $k$-formas determina un isomorfismo entre la cohomología singular de $M$ y su cohomología de de Rham. Esta versión abstracta puede probarse con la tecnología de la teoría de haces: basta ver que el complejo de de Rham proporciona una resolución acíclica del haz constante.
\end{rmk}

\subsection{Estructura de anillo}
La equivalencia entre la cohomología definida axiomáticamente y la cohomología de de Rham nos permite definir un producto en cohomología, el llamado \emph{producto cup}
\begin{equation*}
\smile: H^p(M,\RR) \times H^q(M,\RR) \rightarrow H^{p+q}(M,\RR)\ ([\alpha],[\beta]) \mapsto [\alpha \wedge \beta].
\end{equation*}

Esto nos permite definir el \emph{anillo de cohomología} de $M$, como la suma
\begin{equation*}
H^*(M,\RR)= \bigoplus_{k\in \Z} H^k(M,\RR),
\end{equation*}
equipada con el producto cup. Este anillo es, de hecho, una $\RR$-álgebra
...
\item
          \begin{equation*}
            H^k(M_i,M_{i-1};\RR) \cong
            \begin{cases}
              0, & \text{si } k\neq i,\\
              \RR^{c_k}, & \text{si } k=i.
            \end{cases}
          \end{equation*}
    \item $H^k(M_i,\RR) = 0$, para $i< k$;
    \item $H^k(M_i,\RR) \cong H^k(M,\RR)$, para $i>k$.
  \end{enumerate}
\end{prop}

\begin{proof}
  1. Usando el axioma de escisión y el axioma de homotopía, tenemos que
  \begin{equation*}
H^k(M_i,M_{i-1};\RR) \cong \bigoplus_{j=1}^{c_i} H^k(e_j,\partial e_j;\RR),
  \end{equation*}
  con cada $e_j \cong \RR^i$.
  Por tanto, basta calcular el grupo de cohomología $H^k(D^i,\partial D^i;\RR)$, donde $D^i$ es un $i$-disco cerrado. Si $i,k>1$, podemos usar el axioma de exactitud y el hecho de que $H^k(D^i,\RR) = 0$ para obtener un isomorfismo $H^k(D^i,\partial D^i;\RR) \cong H^{k-1}(S^{i-1},\RR)$. Tomamos ahora un disco cerrado $D^{i-1}\subset S^{i-1}$, embebido como un hemisferio sur, y tomamos $U\subset S^{i-1}$ el complemento del polo sur. Como $U$ retrae por deformación a un punto, aplicando el axioma de exactitud, tenemos
  \begin{equation*}
H^{k-1}(S^{i-1},U;\RR) \cong H^{k-1}(S^{i-1},\RR).
  \end{equation*}
  Ahora, por el axioma de escisión y por el axioma de homotopía, usando que $U\cap D^{i-1}$ retrae por deformación al borde $\partial D^{i-1}$,
  \begin{equation*}
H^{k-1}(S^{i-1},U;\RR) \cong H^{k-1}(D^{i-1},\partial D^{i-1};\RR).
  \end{equation*}
  Obtenemos una serie de isomorfismos
  \begin{equation*}
H^{k}(D^i,\partial D^i;\RR) \cong H^{k-1}(D^{i-1},\partial D^{i-1};\RR) \cong \cdots \cong H^{k-i+2}(D^2,\partial D^2;\RR) \cong H^{k-i+1}(D^1,\partial D^1;\RR).
  \end{equation*}
  Terminamos calculando los grupos de cohomología $H^p(D^1,\partial D^1;\RR)$. Por el axioma de exactitud, obtenemos una sucesión exacta
  \begin{equation*}
      \begin{tikzcd}
H^{p-1}(D^1,\RR)  \rar & H^{p-1}(\partial D^1,\RR) \rar & H^{p}(D^1,\partial D^1;\RR) \rar & H^{p}(D^1,\RR) .
      \end{tikzcd}
  \end{equation*}
 Ahora, como $D^1$ retrae por deformación a un punto y $\partial D^1$ son dos puntos, concluimos que $H^p(D^1,\partial D^1;\RR)=0$ para $p\neq 1$. Para $p=1$, tenemos una sucesión exacta
\begin{equation*}
  \begin{tikzcd}
\RR \ar{rr}{a \mapsto (a,-a)} && \RR \oplus \RR \rar & H^1(D^1,\partial D^1;\RR) \rar & 0,
  \end{tikzcd}
\end{equation*}
de modo que $H^1(D^1,\partial D^1;\RR) \cong \RR^2 / \left\{ (a,-a) : a\in \RR \right\} \cong \RR$.

2. Tomamos ahora la sucesión exacta asociada al par $(M_i,M_{i-1})$ y obtenemos una sucesión exacta
\begin{equation*}
    \begin{tikzcd}
H^{k}(M_i,M_{i-1};\RR) \rar & H^{k}(M_i,\RR) \rar & H^{k}(M_{i-1},\RR) \rar & H^{k+1}(M_i,M_{i-1};\RR).
    \end{tikzcd}
\end{equation*}
Ahora, si $i<k$, por el punto 1, los espacios de los extremos son nulos y obtenemos un isomorfismo $H^k(M_i,\RR)\cong H^k(M_{i-1},\RR)$. Iterando este argumento, obtenemos un isomorfismo
\begin{equation*}
H^k(M_i,\RR) \cong H^k(M_0,\RR) \cong 0.
\end{equation*}

3. En primer lugar, podemos asumir que $k\leq n$, ya que para $k>n$ ambos grupos son $0$. De nuevo, consideramos la sucesión exacta asociada al par $(M_i,M_{i-1})$. Para $i>k+1$, los espacios de los extremos son nulos y obtenemos un isomorfismo $H^k(M_i,\RR)\cong H^k(M_{i-1},\RR)$. En particular, como $M=M_n$, tenemos un isomorfismo
\begin{equation*}
H^k(M,\RR) \cong H^k(M_{i},\RR),
\end{equation*}
para todo $i> k$.
\end{proof}

Los grupos $H^k(M_k,M_{k-1};\RR)$ encajan en el siguiente diagrama
\begin{center}
...
& &H^{k-1}(M_{k-1},\RR) \ar{ru} & && & H^{k+1}(M_{k+1},\RR).
  \end{tikzcd}
  \end{adjustbox}
\end{center}
...
\begin{equation*}
\chi(M)= \sum_{k} (-1)^k \dim CH^k(M,\RR) =\sum_{k} (-1)^k \dim H^k(M,\RR).
\end{equation*}
...
\begin{ej}
  ...
  \item \textbf{Esferas}. Una $n$-esfera $S^n$ admite una descomposición celular con dos celdas: una $0$-celda y una $n$-celda. Por tanto, su complejo celular $CH^\bullet(S^n,\RR)$ es
          \begin{center}
            \begin{tikzcd}
0 \rar & \RR \rar & 0 \rar & \cdots \rar & 0  \rar & \RR \rar & 0.
            \end{tikzcd}
          \end{center}
          Por tanto,
          \begin{equation*}
...
\subsection{Cohomología con soportes compactos} Los \emph{grupos de cohomología con soporte compacto} de un espacio topológico $X$ (con coeficientes en $\RR$), se definen como límites directos
\begin{equation*}
H_c^k(X,\RR) := \varinjlim_{K\subset X \text{compacto}} H^k(X,X\setminus K; \RR).
\end{equation*}

Para una variedad diferenciable $M$, tenemos los espacios de formas diferenciales con soporte compacto
\begin{equation*}
\Omega^k_c(M) = \left\{ \alpha \in \Omega^k(M): \mathrm{sop}(\alpha) \subset K,\ K\subset M  \text{ compacto} \right\},
\end{equation*}
...
H^k_{\mathrm{dR},c}(M) = H^k(\Omega_c^\bullet(M)) = \frac{\ker(d:\Omega_c^k(M)\rightarrow \Omega_c^{k+1}(M))}{d(\Omega_c^{k-1}(M))}.
  \end{equation*}
  Con las mismas técnicas que demuestran el teorema de de Rham abstracto se obtiene un isomorfismo
  \begin{equation*}
...
\begin{proof}
  La esfera $S^n$ es la compactificación de Alexandroff de $\RR^n$, de modo que
\begin{equation*}
H_c^k(\RR^n,\RR) := \varinjlim_{K\subset \RR^n \text{compacto}} H^k(\RR^n,\RR^n \setminus K; \RR) = H^k(S^n,\infty;\RR).
\end{equation*}
...
Si $M$ es una variedad diferenciable orientable, el emparejamiento
  \begin{equation*}
...
\begin{rmk}
De nuevo, la tecnología de la teoría de haces permite hacer este paso local-global, y es la herramienta principal detrás de la prueba de la dualidad de Poincaré global. La orientabilidad de la variedad es crucial. En efecto, en general el ``dual'' del haz constante es el \emph{haz de orientaciones}, que es isomorfo al haz constante si y sólo si $M$ admite una orientación.
\end{rmk}

\section{Teoría de Morse y teoría del grado}
\subsection{Funciones de Morse} Otra forma de estudiar la topología de las variedades diferenciables es por medio de las funciones de Morse. Si $M$ es una variedad diferenciable de dimensión $n$ y $f:M\rightarrow \RR$ es una función, se denomina un \emph{punto crítico} de $f$ a un punto $x\in M$ tal que $d_xf =0$. Se denota por $\Crit(f)\subset M$ el conjunto de los puntos críticos de $f$. La imagen $\VC(f)\subset \RR$ de este conjunto es el conjunto de los \emph{valores críticos}. El \emph{hessiano} de $f$ en un punto crítico $x\in \Crit(f)$ es la forma bilineal $\Hess_x(f):T_xM\times T_xM \rightarrow \RR$ definida por
\begin{equation*}
\Hess_x(f)(X_x,Y_x) = X_x\circ Y_x(f) = Y_x\circ X_x(f).
\end{equation*}
Nótese que, como $x$ es un punto crítico, tenemos que $[X,Y]_x(f)=df([X,Y]_x)=0$.
...
\end{equation*}
Ahora, para cada $k$ tenemos una sucesión exacta larga
\begin{center}
  \begin{tikzcd}[column sep=1em]
\cdots \rar & H^{q}(M_{k+1},M_k;\RR) \rar & H^q(M_{k+1},\RR) \rar & H^q(M_k,\RR) \rar & H^{q+1}(M_{k+1},M_k;\RR) \rar & \cdots.
  \end{tikzcd}
\end{center}
Por tanto,
\begin{equation*}
\chi(M_{k+1})- \chi(M_k) = \sum_{i} (-1)^i \dim H^i(M_{k+1},M_k;\RR).
\end{equation*}
...
De igual manera, una métrica Riemanniana en una variedad $M$ determina el denominado \emph{isomorfismo musical} $TM \rightarrow T^*M$, que envía cada campo vectorial $X \in \fX(M)$ a la $1$-forma $X^\sharp=g(X,-) \in \Omega^1(M)$. Componiendo con este isomorfismo, se obtiene un tensor simétrico $g^\sharp$ de tipo $(2,0)$, denominado el \emph{tensor métrico inverso}. En coordenadas, denotamos
\begin{equation*}
...
0 \rar & V \rar & \mathrm{Euc}(E) \rar & \mathrm{O}(V,\langle-,-\rangle) \rar & 1.
  \end{tikzcd}
\end{center}
El grupo $\mathrm{Euc}(n)=\mathrm{Euc}(\RR^n)$ del espacio euclidiano estándar se denomina el grupo euclidiano.
...
\end{equation*}
Esta aplicación es diferenciable, por construcción, y su diferencial en un punto $x\in C$ es la llamada \emph{aplicación de Weingarten}
\begin{align*}
S_x=- d_x\bm{n} : T_xC  \rightarrow T_{\bm{n}(p)} S(V).
\end{align*}
El espacio tangente $T_{\bm{n}(p)}S(V)$ está generado por el vector tangente unitario $\bm{t}_x$, de modo que podemos escribir
\begin{equation*}
S_x(\bm{t}_x) = \lambda \bm{t}_x,
\end{equation*}
para un cierto número $\lambda \in \RR$, pero resulta que este número es precisamente $\kappa(p)$. En efecto,
\begin{equation*}
\lambda \bm{t}_x =-d_x\bm{n}(\bm{t}_x) =- (d\bm{n})_x(\bm{t}_x) = \alpha_x(\bm{t}_x) \bm{t}_x = \kappa (i^*\theta^1)_x(\bm{t}_x) \bm{t}_x = \kappa \bm{t}_x.
\end{equation*}
...
\begin{equation*}
\kappa_x: \mathbb{P}(T_x S) \rightarrow \RR,
\end{equation*}
que asigna a cada recta $L\subset T_xS$ la curvatura de la curva $S\cap \Pi(L)$, obtenida como la intersección de $S$ con el plano $\Pi(L)\subset E$ que contiene a $L$ y pasa por $p$.

Por el teorema de la función inversa, podemos ver localmente la superficie $S$ como la gráfica de una función $F:T_xS\rightarrow \RR$ con $F(0)=0$ y $d_0 F=0$. Se denomina la \emph{segunda forma fundamental} al hessiano
\begin{equation*}
\mathbf{II}_x:=\mathrm{Hess}_x F: T_xS \times T_x S \rightarrow \RR.
\end{equation*}
La función $\kappa_x$ mide la segunda derivada de $F$, en el sentido de que se tiene, para cada recta $L\subset T_xS$, y para cada vector unitario $v\in L$,
\begin{equation*}
\mathbf{II}_x(v,v) = \kappa_x(L).
\end{equation*}

Como la primera forma fundamental $\mathbf{I}_x=g_x$ es no degenerada, ha de existir un operador $S_x:T_xS \rightarrow T_xS$, llamado el \emph{operador de forma}, de modo que
\begin{equation*}
\mathbf{II}_x(v,w) = \mathbf{I}_x(v,S_x(w)).
\end{equation*}

\begin{lemma}
El operador de forma $S_x$ coincide con la aplicación de Weingarten.
\end{lemma}

\begin{proof}
  Tomamos ``coordenadas euclidianas'' $x$ y $y$ en el espacio afín tangente $x+T_x S \subset E$ y definimos $F=F(x,y)$ de modo que en un entorno de $x$, tenemos que $S$ es la gráfica de $F$. Entonces
  \begin{equation*}
...
\end{pmatrix} \right|_x = - \mathrm{Hess}_x F,
  \end{equation*}
  que es la matriz asociada a $\mathbf{II}_x$ en estas coordenadas euclidianas.
\end{proof}

\subsection{} Como la segunda forma fundamental es simétrica, el operador de forma $S_x$ es autoadjunto, por lo que es diagonalizable y tiene autovalores reales $\lambda_1,\lambda_2 \in \RR$. Distinguimos dos casos:
\begin{enumerate}
\item Si $\lambda_1=\lambda_2=\lambda$, entonces $S_x = \lambda \mathrm{id}_{T_x S}$. Decimos entonces que $x$ es un \emph{punto umbilical}.
  \item Si $\lambda_1\neq \lambda_2$ (supongamos, sin pérdida de generalidad que $\lambda_1>\lambda_2$), entonces el espacio $T_xS$ descompone ortogonalmente como $T_xS=L_1\oplus L_2$, donde los $L_i$ son los autoespacios, llamados \emph{líneas principales}, con autovalores $\lambda_i$, llamados \emph{curvaturas principales}. Se tiene que $\lambda_1$ es el máximo de la función $\kappa_x$, que se alcanza en $L_1$, mientras que $\lambda_2$ es su mínimo, que se alcanza en $L_2$.
\end{enumerate}

Al cambiar la orientación normal, se intercambian $\kappa_x \mapsto -\kappa_x$ y $\lambda_i\mapsto -\lambda_i$. Se denomina \emph{curvatura media} a la función
\begin{equation*}
H(x) := \frac{\lambda_1 + \lambda_2}{2} = \frac{1}{2} \mathrm{tr}(S_x).
\end{equation*}
Al cambiar la orientación normal, tenemos $H\mapsto -H$. Por otra parte, tenemos la \emph{curvatura de Gauss}
\begin{equation*}
K(x) := \lambda_1 \lambda_2 = \det(S_x),
\end{equation*}
...
\begin{thm}[Teorema egregio de Gauss]
  Dada una superficie $S\subset E$ y un punto $x\in S$, se tiene
  \begin{equation*}
K(x) = R^1_{212}(x),
  \end{equation*}
...
\begin{proof}
  En un entorno de $x$ podemos ver la superficie $S$ como la gráfica de una función $(x^1,x^2)\mapsto (x^1,x^2,F(x^1,x^2))$, de modo que tenemos
  \begin{equation*}
...
\begin{defn}
La \emph{derivada covariante} $\nabla_{v_x}X$ de un campo vectorial $X\in \fX(S)$ a lo largo de una dirección $v_x\in T_xS$ es la proyección de $D_{v_x}X \in V$ a $T_xS$.
\end{defn}
...
\begin{enumerate}
\item La aplicación $\nabla X: T_xS \rightarrow T_xS$, $v_x \mapsto \nabla_{v_x}X$ es lineal.
\item La aplicación $\nabla_{v_x}: \fX(S) \rightarrow T_xS$, $X \mapsto \nabla_{v_x}X$ satisface la regla de Leibniz
        \begin{equation*}
\nabla_{v_x}(fX) = (v_x\cdot f) X + f \nabla_{v_x}X.
        \end{equation*}
\item $\nabla_{v_x}(X+X')=\nabla_{v_x}(X) + \nabla_{v_x}(X')$.
\item $v_x \cdot \langle X, X' \rangle = \langle \nabla_{v_x}X,X'\rangle + \langle X, \nabla_{v_x}X'\rangle$.
\end{enumerate}
\end{lemma}
...
\section{Curvatura de una variedad riemanniana}
\subsection{Curvatura de Riemann} En secciones previas ya hemos introducido el \emph{tensor de curvatura de Riemann} $R\in \mathcal{T}^1_3(M)$ asociado a una variedad riemanniana $(M,g)$, que en coordenadas locales se expresa como
\begin{equation*}
R=R^i_{jkl} \partial_i \otimes dx^j \otimes dx^k \otimes dx^l,
\end{equation*}
donde los términos $R^i_{jkl}$ se definen a partir de los coeficientes de Christoffel como sigue
\begin{equation*}
R^i_{jkl} = \partial_{k}\Gamma^i_{jl} - \partial_l \Gamma^i_{jk} + \Gamma^m_{jl} \Gamma^i_{mk} - \Gamma^m_{jk} \Gamma^i_{ml}.
\end{equation*}

Vamos a dar ahora una definición intrínseca del tensor $R$, a partir de la conexión de Levi-Civita. Como $R$ es un tensor de tipo $(1,3)$, puede definirse como una aplicación $C^\infty(M)$-lineal
\begin{equation*}
R: \fX(M)\times \fX(M) \times \fX(M) \rightarrow \fX(M),\ (X,Y,Z) \mapsto R(X,Y) Z.
\end{equation*}
Ponemos
\begin{equation*}
R(X,Y)Z:= \nabla_X \nabla_Y Z - \nabla_Y \nabla_X Z - \nabla_{[X,Y]}Z.
...
\subsection{Interpretación geométrica} Una posible interpretación intuitiva de la curvatura es como sigue. Fijamos un punto $x\in M$. Si nos movemos ligeramente en la dirección de $X_x$, siguiendo su flujo, y después en la dirección de $Y_x$, tenemos una pequeña discrepancia con movernos primero en la direección de $Y_x$ y luego en la dirección de $X_x$. Esta pequeña discrepancia está dada precisamente por el corchete $[X,Y]_x$. La curvatura $R(X,Y)Z$ mide la no-conmutatividad de las derivadas covariantes $\nabla_X$ y $\nabla_Y$, y corrige con la derivada $\nabla_{[X,Y]}$ (para poder cerrar el circuito).

\subsection{Simetrías del tensor de curvatura} Es habitual contraer la curvatura con la métrica para obtener un tensor de tipo $(0,4)$. En coordenadas,
$R = R_{ijkl} dx^i \otimes dx^j \otimes dx^k \otimes dx^l$,
con $R_{ijkl}=R^m_{ijk}g_{ml}$. Globalmente,
\begin{equation*}
R(X,Y,Z,T) = \langle R(X,Y)Z,T \rangle.
\end{equation*}
A priori, el tensor $R$ está determinado por $n^4$ funciones, donde $n=\dim M$, pero muchas de estas son redundantes, ya que tiene varias simetrías.

\begin{prop}
  Dados campos vectoriales $X,Y,Z,T\in \fX(M)$, se tiene
  \begin{enumerate}
    \item $R(X,Y,Z,T)=-R(Y,X,Z,T)$,
    \item $R(X,Y,Z,T)=-R(X,Y,T,Z)$,
    \item $R(X,Y,Z,T)=R(Z,T,X,Y)$,
    \item $R(X,Y,Z,T)+R(Y,Z,X,Y) + R(Z,X,Y,T)=0$ (\emph{identidad de Bianchi}).
  \end{enumerate}
\end{prop}

\subsection{Curvatura seccional} Las dos primeras propiedades anteriores implican que, para $x\in M$, tenemos \[R_x \in \Lambda^2(T^*_xM) \otimes \Lambda^2(T^*_xM).\] La tercera propiedad da la simetría entre estos dos factores, de modo que, de hecho $R_x \in S^2(\Lambda^2 T_x^*M)$. Por tanto, $R_x$ está determinado por la forma cuadrática correspondiente
\begin{equation*}
\Lambda^2(T_xM) \rightarrow \RR,\  u\wedge v \mapsto R_x(u,v,u,v).
\end{equation*}
Esto motiva la introducción de la \emph{curvatura seccional}, que tiene la misma información que la curvatura de Riemann.

\begin{defn}
  Sea $x\in M$ un punto, $u,v \in T_xM$ vectores linealmente independientes, y $\sigma \subset T_xM$ el plano que generan. Se define la \emph{curvatura seccional} de $\sigma$ como
  \begin{equation*}
K_x(\sigma) = -\frac{R_x(u,v,u,v)}{\lVert u \wedge v \rVert^2},
  \end{equation*}
  donde $\lVert u \wedge v \rVert^2 = \lVert u\rVert^2\lVert v\rVert^2- \langle u,v \rangle^2$, es el cuadrado del área del paralelogramo determinado por $u$ y $v$.
\end{defn}

\begin{rmk}
  En una superficie $S$, podemos tomar una base ortogonal del plano tangente $e_1,e_2\in T_xS$, y calcular su curvatura seccional en el punto $x$ como
  \begin{equation*}
K_x(T_xS) = -R_x(e_1,e_2,e_1,e_2) = R^1_{212}(x),
  \end{equation*}
que es igual a la curvatura de Gauss $K(x)$, por el teorema egregio de Gauss.

Se puede ver que, de hecho, la curvatura es esencialmente un fenómeno ``en dimensión 2'', ya que la curvatura seccional de $M$ se puede recuperar de la curvatura de ciertas superficies en $M$.
\end{rmk}

\begin{prop}
Sea $x\in M$ y $\sigma \subset T_xM$ un plano. Consideremos una bola $B^d_{\varepsilon}(0)\subset T_xM$ y la superficie $S=\exp_x(\sigma \cap B_{\varepsilon}(0)) \subset M$. Entonces, la curvatura seccional $K_x(\sigma)$ coincide con la curvatura de Gauss de $S$ en $x$.
\end{prop}

\begin{proof}
Por construcción de $S$, podemos hallar unas coordenadas $x^1,x^2,\dots,x^n$ en un entorno de $x$, de modo que $S$ se identifique en este entorno con la superficie dada por la ecuación $x^3=\cdots=x^n =0$. En estas coordenadas, la restricción $\bar{g}=g|_S$ de la métrica $g$ a $S$ es simplemente \[\bar{g}(x^1,x^2)=g_{ij}(x^1,x^2,0,\dots,0) dx^i\otimes dx^j, \]
para $i,j=1,2$. Por tanto, la curvatura de Gauss de la métrica restringida es
\begin{equation*}
K(x)=\bar{R}^{1}_{212} (x) = R^1_{212}(x) = -R_x(\partial_1,\partial_2,\partial_1,\partial_2) = K_x(\sigma),
\end{equation*}
como queríamos probar.
\end{proof}

\subsection{Curvatura de Ricci} Otro tensor interesante es el \emph{tensor de Ricci}, un tensor de tipo $(0,2)$ construido a partir de la curvatura, y definido como sigue. Para cada $x\in M$, definimos la aplicación
\begin{equation*}
\mathrm{Ric}_x: T_xM \times T_xM \rightarrow \RR
\end{equation*}
como
\begin{equation*}
\mathrm{Ric}_x(Y,Z) := \frac{1}{n-1} \mathrm{tr}(X\mapsto R_x(X,Y)Z).
\end{equation*}
Equivalentemente,
\begin{equation*}
\mathrm{Ric}_x(Y,Z) = \frac{1}{n-1} \sum_i R_x(Y,e_i,Z,e_i),
\end{equation*}
donde $(e_1,e_2,\dots,e_n)$ es una base ortogonal de $T_xM$. En coordenadas, $\mathrm{Ric}=R_{ij} dx^i\otimes dx^j$, para
\begin{equation*}
\mathrm{Ric}_{ij} = \frac{1}{n-1} R^{k}_{jki} = \frac{1}{n-1} R^k_{ikj}.
\end{equation*}

Como el tensor de Ricci es simétrico, está determinado por la \emph{curvatura de Ricci}
\begin{equation*}
\mathrm{Ric}_x(X) = \mathrm{Ric}_x(X,X).
\end{equation*}
Si $X$ es un vector unitario y tomamos una base ortonormal $e_1=X,e_2,\dots,e_n$, y denotamos por $\sigma_{j}$ el plano generado por $X$ y $e_j$, tenemos que
\begin{equation*}
\mathrm{Ric}_x(X) = \frac{1}{n-1} \sum_{j=2}^n K_x(\sigma_j)
\end{equation*}
es el promedio de las curvaturas seccionales de los planos que contienen a $X$. También puede calcularse como una integral
\begin{equation*}
\mathrm{Ric}_x(X) = \frac{1}{\mathrm{vol}(S^{n-2})} \int_{u \in S^{n-2}} K_x(\sigma_u),
\end{equation*}
donde $u$ es el plano generado por $X$ y $u$. Aquí, por $S^{n-2}$ entendemos el subconjunto
\begin{equation*}
S^{n-2} = \left\{ u \in T_xM: u \perp X, \lVert u \rVert = 1 \right\}.
\end{equation*}

\subsection{Curvatura escalar} Finalmente, podemos considerar la \emph{curvatura escalar}, que es ($\tfrac{1}{n}$ por) la traza del tensor de Ricci con respecto de la curvatura; esto es
\begin{equation*}
\mathrm{Scal}(x) = \frac{1}{n} \sum_{i=1}^n \mathrm{Ric}_x(e_i,e_i).
\end{equation*}
 En coordenadas
\begin{equation*}
\mathrm{Scal} = g^{ij} R_{ij}.
\end{equation*}
Equivalentemente, $\mathrm{Scal}(x)$ es el promedio de todas las curvaturas seccionales en $x$, esto es
\begin{equation*}
\mathrm{Scal}(x) = \frac{1}{n(n-1)} \sum_{i\neq j} K_x(\sigma_{ij}),
\end{equation*}
para $\sigma_{ij}$ el plano generado por los vectores $e_i$ y $e_j$ de una base ortonormal $e_1,\dots,e_n$ de $T_xM$.
...
\item Si $V$ es un espacio vectorial de dimensión finita, entonces $(V,+)$ es un grupo de Lie (conmutativo).
  ...
  \item Si $V$ está equipado con un producto escalar, podemos considerar el \emph{grupo ortogonal} $\mathrm{O}(V)$ de automorfismos de $V$ que preservan el producto escalar, y su subgrupo $\mathrm{SO}(V)\subset \mathrm{O}(V)$ formado por aquellos automorfismos con determinante $1$, denominado el \emph{grupo ortogonal especial}. Cuando $V=\RR^n$ con el producto escalar estándar, entonces $\mathrm{O}(n)=\mathrm{O}_n(\RR)=\mathrm{O}(\RR^n)$ es el grupo de las matrices ortogonales: matrices $n\times n$ que cumplen la condición
        \begin{equation*}
A^T A = \mathbb{I}_n.
        \end{equation*}
        También podemos considerar el subgrupo $\mathrm{SO}(n)=\mathrm{SO}_n(\RR)\subset \mathrm{O}(n)$ de las matrices ortogonales con determinante $1$.
...
A^T J A = J,
  ...
  \item Si $V$ es un espacio vectorial complejo equipado con una forma hermítica $h:V\times V\rightarrow \mathbb{C}$, es decir, con una forma sesquilineal ($h(\lambda v,w)=\lambda h(v,w)$, $h(v,\lambda w)=\bar{\lambda} h(v,w)$), podemos considerar el \emph{grupo unitario} $\mathrm{U}(V)$ de automorfismos de $V$ que preservan $h$. Cuando $V=\mathbb{C}^n$ está equipado con la forma hermítica estándar
        \begin{equation*}
h((x_1,\dots,x_n),(y_1,\dots,y_n)) = \sum_{i=1}^n x_i \bar{y}_i,
        \end{equation*}
        entonces $\mathrm{U}(n):= \mathrm{U}(\mathbb{C}^n)$ es el grupo de las matrices unitarias: matrices complejas $n\times n$ que cumplen la condición
        \begin{equation*}
A^\dagger A = \mathbb{I}_n,
        \end{equation*}
donde $A^\dagger$ es la matriz traspuesta conjugada de $A$. También se puede considerar el grupo $\mathrm{SU}(n)\subset \mathrm{U}(n)$ de las matrices unitarias con determinante $1$.
\end{itemize}
\end{ex}

\begin{rmk}
        Los grupos $\GL_n$, $\mathrm{SL}_n$, $\mathrm{O}_n$, $\mathrm{SO}_n$, $\mathrm{Sp}_n$ forman la clase de los \emph{grupos clásicos}. Nótese además que todos estos grupos pueden definirse de forma análoga sobre cualquier cuerpo o incluso sobre $\mathbb{Z}$. De hecho, es fácil ver que todas las condiciones que determinan estos grupos son polinomiales, de modo que éstos son ejemplos de \emph{grupos algebraicos}. El hecho de que se puedan definir sobre $\mathbb{Z}$ y que para cualquiera de estos grupos $G$ y para cualquier otro anillo $R$ se pueda recuperar el grupo sobre $R$ cambiando de base el grupo sobre $\mathbb{Z}$, de modo que $G(R) = G_{R}(R)$, significa que estos son \emph{grupos escindidos}.

        Nótese además que los grupos $\mathrm{U}(n)$, $\mathrm{SU}(n)$, $\mathrm{O}(n)$ y $\mathrm{SO}(n)$ son compactos, de hecho, son los grupos compactos maximales de los grupos $\mathrm{GL}_n(\mathbb{C})$, $\mathrm{SL}_n(\mathbb{C})$, $\mathrm{GL}_n(\RR)$ y $\mathrm{SL}_n(\RR)$, respectivamente. También se puede probar que el compacto maximal del grupo simpléctico real $\mathrm{Sp}_{2n}(\RR)$ es también el grupo unitario $\mathrm{U}(n)$. Por otra parte, el compacto maximal del grupo simpléctico complejo $\mathrm{Sp}_{2n}(\mathbb{C})$ es otro grupo $\mathrm{Sp}(n)$, denominado el \emph{grupo simpléctico compacto}.
\end{rmk}
...
\subsection{Métricas riemannianas} Dar una métrica riemanniana en una variedad $M$ equivale a dar una reducción del grupo de estructura de su fibrado de referencias $\cB(M)$ a $\OO(n)\subset \GL_n(\RR)$. En efecto, toda matriz invertible $A$ descompone de forma única como
        \begin{equation*}
A= QS,
        \end{equation*}
        donde $Q\in \OO(n)$ es una matriz ortogonal y $S$ es una matriz simétrica definida positiva. Por tanto, podemos identificar el cociente $P=\GL_n(\RR)/\OO(n)$ con el conjunto de las matrices simétricas definidas positivas, de modo que dar una sección de $\cB(M)\times^{\GL_n(\RR)} P$ equivale precisamente a definir un producto escalar en cada espacio tangente, de forma consistente con la estructura diferenciable; es decir, equivale a dar una métrica riemanniana.
Más generalmente, se puede definir una noción de \emph{métrica riemanniana} en todo fibrado vectorial $V\rightarrow M$ como una reducción del grupo de estructura de $\cB(V)$ a $\OO(n)$.
...
\subsection{Forma de volumen} Dar una forma de volumen en $M$ equivale a dar una reducción del grupo de estructura de $\cB(M)$ a $\mathrm{SL}_n(\RR)$. En particular, una forma de volumen determina una orientación, ya que $\mathrm{SL}_n(\RR)\subset \GL_n(\RR)^+$. A su vez, una métrica riemanniana en una variedad orientada determina una forma de volumen ya que \[\OO(n) \cap \GL_n(\RR)^+=\OO(n)\cap \mathrm{SL}_n(\RR)= \SO(n).\]
En otras palabras, una orientación junto con una métrica riemanniana determinan una reducción del grupo de estructura de $\cB(M)$ a $\SO(n)$.
...
\subsection{Estructuras de spin} Un ejemplo sencillo de extensión central es el grupo de spin. Por definición, para $n>2$, se define el correspondiente \emph{grupo de spin real} $\mathrm{Spin}(n)=\mathrm{Spin}_n(\RR)$ como el recubridor universal del grupo $\SO(n)$. Resulta que el grupo fundamental de $\SO(n)$ es $\Z/2\Z$ (para $n=3$ se prueba a mano porque $\SO(3)\cong \mathbb{P}^3_{\RR}$, y para $n>3$ se deduce del caso $n=3$ por la sucesión exacta larga en homotopía; otra forma de probarlo es usando sistemas de raíces, ya que $\SO_n$ es un grupo escindido). Por tanto, se tiene una sucesión exacta corta
\begin{center}
  \begin{tikzcd}
1\rar & \Z/2\Z \rar & \mathrm{Spin}(n) \rar & \SO(n) \rar & 1,
  \end{tikzcd}
\end{center}
...
\begin{adjustbox}{max width=\textwidth}
\begin{tikzcd}[column sep = 10pt]
H^1(M,\Z/2\Z) \rar & \check{H}^1(M,\{U_i,i\in I\}, \mathrm{Spin}(n)) \rar & \check{H}^1(M,\{U_i,i\in I\},\SO(n)) \rar & H^2(M,\Z/2\Z),
\end{tikzcd}
\end{adjustbox}
\]
donde $M=\coprod_{i\in I} U_i$ es un recubrimiento tal que cada $\mathrm{Spin}(n)(U_i)\rightarrow \SO(n)(U_i)$ es sobreyectiva.

Una \emph{estructura de spin} en una variedad riemanniana orientada $(M,g)$ de dimensión $n$ es un $\mathrm{Spin}(n)$-fibrado principal $\cB_{\mathrm{Spin}}(M)$ cuya imagen por la aplicación en la sucesión exacta anterior es el fibrado de referencias ortonormales positivas $\cB_{\mathrm{SO}}(M)$. La imagen $w_2(P)\in H^2(M,\Z/2\Z)$ de un $\SO(n)$-fibrado principal $P$ por la aplicación anterior se denomina la \emph{segunda clase de Stiefel--Whitney}. En particular, la segunda clase de Stiefel--Whitney de $M$ se define como $w_2(M)=w_2(\cB_{\SO}(M))$. De la sucesión exacta vemos que $M$ admite una estructura de spin si y sólo si $w_2(M)=0$. Además, la estructura de spin, si existe, no es necesariamente única, sino que hay tantas estructuras de spin diferentes como clases en $H^1(M,\Z/2\Z)$.
...
\subsection{Geometría euclidiana a la Klein} Recordemos el grupo euclidiano $\mathrm{Euc}(n)$, definido como el grupo de automorfismos afines del espacio euclidiano estándar que preservan el producto escalar. Dado un punto del espacio euclidiano, se tiene una inclusión $\OO(n)\rightarrow \mathrm{Euc}(n)$ como el grupo de transformaciones que preservan el punto y el producto escalar con respecto a ese punto. Podemos identificar entonces el espacio cociente $\mathrm{Euc}(n)/\OO(n)$ con el espacio vectorial $\RR^n$. El par $(\mathrm{Euc}(n),\OO(n))$ determina una geometría de Klein, denominada la \emph{geometría euclidiana}.

\subsection{Geometría riemanniana a la Cartan} Ahora, a una variedad riemanniana $(M,g)$ se le puede asociar el fibrado de referencias ortonormales $\pi:\cB_{\OO}(M)\rightarrow M$. Este fibrado está equipado con la conexión pricipal inducida por la conexión de Levi-Civita, cuya $1$-forma de conexión denotamos por $A \in \Omega^1(\cB_{\OO}(M),\so(n))$. Además, al tratarse de una conexión afín, el fibrado de referencias está equipado también con la \emph{$1$-forma de soldadura} $\theta=(\theta^1,\dots,\theta^n)\in \Omega^1(\cB_{\OO}(M),\RR^n)$, definida como sigue. Si $\bm{e} \in \cB_{\OO}(M)$ es una referencia ortogonal $\bm{e}:\RR^n\rightarrow T_xM$ en el punto $x=\pi(\bm{e})$ y $X\in T_{\bm{e}}\cB_{\OO}(M)$ es un vector tangente, definimos
\begin{equation*}
...
La suma $\omega:= A + \theta \in \Omega^1(\cB_{\OO}(M),\mathfrak{euc}(n))$ determina una $1$-forma en $\cB_{\OO}(M)$ con valores en $\mathfrak{euc}(n)=\so(n) \oplus \RR^n$ y por tanto el par $(\cB_{\OO}(M),\omega)$ es una geometría de Cartan modelada sobre la geometría euclidiana $(\mathrm{Euc}(n),\OO(n))$. La curvatura de esta geometría es igual a
\begin{equation*}
...
\subsection{} Supongamos que $(M,g)$ es una $n$-variedad riemanniana con curvatura seccional constante $K\in \RR$. Definimos el álgebra de Lie $\fg_K$ con espacio subyacente $\so(n)\oplus \RR^n$ y equipado con el corchete
\begin{equation*}
...
\end{equation*}
De modo que $\omega$ puede verse como una $1$-forma valorada en $\fg_K$, y $(\cB_{\OO}(M),\omega)$ determina una geometría de Cartan plana modelada sobre $(G_K,\SO(n))$, donde $G_K$ es un grupo de Lie con álgebra de Lie $\fg_K$.

\subsection{Geometrías de curvatura constante} Describamos ahora el espacio cociente $G_K/\SO_n(\RR)$.
\subsubsection{Curvatura nula} Si $K=0$, entonces $\fg_K$ es isomorfo al álgebra $\mathfrak{euc}(n) = \so(n)\oplus \RR^n$, de modo que $G_K$ es el grupo euclidiano $\mathrm{Euc}(n)$ y $G_K/\SO_n(\RR)$ es el espacio euclidiano estándar $n$-dimensional, que denotamos por $\mathbb{E}^n$.
...
\end{equation*}
que envía cada par $(A,a) \in \so(n) \oplus \RR^n$ a la matriz
\begin{equation*}
  i(A,a)=
\begin{pmatrix}
  0 & a^T \\
...
\subsubsection{Curvatura positiva} Si $K>0$, el grupo $G_K$ es isomorfo a $\SO(n+1)$, y el cociente $G_K/\SO(n)$ se identifica con la esfera $n$-dimensional de radio $R$, definida como
\begin{equation*}
...
\subsubsection{Curvatura negativa} Si $K<0$, el grupo $G_K$ es isomorfo al grupo $\SO(1,n)$, y el cociente $G_K/\SO(n)$ se identifica con el espacio hiperbólico $n$-dimensional de radio $R$ definido como la hoja superior de la superficie
\begin{equation*}
...
\begin{prop}
Una $n$-variedad riemanniana $(M,g)$ con curvatura seccional constante $K\in \RR$ es localmente isométrica al espacio $G_K/\SO(n)$.
\end{prop}
...
\begin{proof}
  ...
  \begin{center}
    \begin{tikzcd}
     ...
     \widetilde{M} \ar{r} & G_K/\SO(n),
    \end{tikzcd}
  \end{center}
donde $\varpi:\widetilde{M}\rightarrow M$ es el espacio recubridor universal, equipado con la métrica $\varpi^* g$. Cualquier sección local $s:U\subset M\rightarrow \tilde{M}$ de $\varpi$ determina una isometría entre $U$ y $\varpi^{-1}(U)$, y por tanto una isometría local a $G_K/\SO(n)$. Ahora, la existencia del isomorfismo entre $(\cB_{\OO}(\widetilde{M}),\omega)$ y $(G_K,G_K/\SO(n))$ se sigue de \todo{REF}, ya que $\omega$ es plana.
\end{proof}
...
\chapter{Introducción a las clases características}

\section{Polinomios invariantes}
\subsection{} Sea $V$ un espacio vectorial sobre $\RR$. Recordemos que habíamos definido el álgebra simétrica $S(V)=\bigoplus_{k=0}^\infty S^k(V)$, y que esta se podía identificar con el álgebra polinomial $\RR[V]$, cuyos elementos son funciones polinomiales en los elementos $x_1,\dots,x_n$ de una base del espacio dual $V^*$. Supongamos que $G$ es un grupo que actúa en $V$ por transformaciones lineales; es decir, que existe un homomorfismo de grupos $G\rightarrow \GL(V)$. Definimos la $\RR$-álgebra de funciones polinomiales $G$-invariantes como el anillo
\begin{equation*}
\RR[V]^G = \left\{ f\in \RR[V]: f(g\cdot v) = f(v), \forall g\in G \right\}.
\end{equation*}

\subsection{} Es particularmente interesante el caso en el que $G$ es un grupo de Lie, $V=\fg$ es su álgebra de Lie y la acción de $G$ en $\fg$ está dada por la representación adjunta
\begin{equation*}
\mathrm{ad}: G \rightarrow \GL(\fg),\ g \rightarrow \ad_g.
\end{equation*}
En este capítulo nos va a interesar dar una descripción explícita del álgebra $\RR[\fg]^G$ de polinomios invariantes por la acción adjunta, para lo que nos será muy útil el siguiente teorema.

\begin{thm}
  Sea $G$ un grupo de Lie y $\fg$ su álgebra de Lie. Sea $G'\subset G$ un subgrupo de Lie y sea $\fg'$ su álgebra de Lie. Supongamos además que
  \begin{equation*}
\fg= \left\{ \ad_g x': g \in G, x'\in \fg'  \right\}.
  \end{equation*}
  Denotamos
  \begin{equation*}
N=\left\{ g \in G: \ad_g \fg' \subset \fg' \right\}.
  \end{equation*}
La aplicación de restricción $\RR[\fg] \rightarrow \RR[\fg']$ determina un isomorfismo $\RR[\fg]^G \rightarrow \RR[\fg']^N$.
\end{thm}

\begin{proof}
Claramente un elemento de $\RR[\fg]^G$ restringido a $\fg'$ está en $\RR[\fg']^N$. Para probar que la aplicación $\RR[\fg]^G\rightarrow \RR[\fg']^N$ es inyectiva, basta ver que para todo elemento $f\in \RR[\fg]^G$ que cumple que
\begin{equation*}
f(x') = 0, \ \text{para todo } x' \in \fg'
\end{equation*}
se tiene que $f=0$. Ahora, por hipótesis, todo elemento $x\in \fg$ es de la forma $\ad_g(x')$, para $g\in G$ y $x'\in \fg'$; por tanto, como $f$ es invariante por la acción adjunta,
\begin{equation*}
f(x) = f(x')=0.
\end{equation*}
Por tanto, $f=0$.
...
\subsection{}
Podemos usar el resultado anterior para obtener generadores del anillo $\RR[\fg]^G$, donde $G$ es uno de los grupos clásicos. Vamos a ver en particular el caso de $G=\mathrm{SO}(n)$; el resto se obtienen de forma similar, y pueden encontrarse en \todo{CITAR:Kobayashi-NomizuVol2}.

Comenzamos considerando un elemento $A\in \mathfrak{so}(n)$ (es decir, una matriz $n\times n$ antisimétrica) y tomando su polinomio característico
\begin{equation*}
p_A(T) = \det(T \mathbb{I}_n - A) = T^n + f_1(A) T^{n-2} + f_2(A) T^{n-4} + \cdots + f_m(A).
\end{equation*}
Esto nos determina unas funciones $f_1,\dots,f_m \in \RR[\mathfrak{so}(n)]$. Como el polinomio característico es invariante por conjugación, estas funciones pertencen a $\RR[\mathfrak{so}(n)]^{\mathrm{SO}(n)}$.  Nótese que, como $(T \mathbb{I}_n - A)^T = T \mathbb{I}_n + A$, tenemos que $p_A(-T)=(-1)^n p_A(T)$, con lo cual el coeficiente que acompaña a cada $T^{n-i}$, con $i$ impar, se anula.

En el caso en que $n=2m$ es par, existe otra función $\mathrm{Pf}\in \RR[\mathfrak{so}(n)]$, denominada el \emph{pfaffiano}, que cumple que
\begin{equation*}
\mathrm{Pf}(A)^2 = f_m(A) = \det(A)
\end{equation*}
y se define como sigue. Si denotamos $A=(a_{ij})_{i,j}$, entonces
\begin{equation*}
\mathrm{Pf}(A) = \frac{1}{2^m m!} \sum_{\sigma \in \mathfrak{S}_{2m}} (-1)^\sigma \prod_{i=1}^m a_{\sigma(2i-1)\sigma(2i)}.
\end{equation*}
Equivalentemente, si definimos la $2$-forma en $\RR^{2m}$
\begin{equation*}
\omega_A = \sum_{i<j} A_{ij} dx^i \wedge dx^j,
\end{equation*}
entonces
\begin{equation*}
\omega_A^{m} = \mathrm{Pf}(A) dx^1 \wedge \cdots dx^{2m}.
\end{equation*}
En particular, para $g\in \mathrm{GL}_{2m}(\RR)$, se tiene que $\mathrm{Pf}(gAg^T)=\det(g)\mathrm{Pf}(A)$, luego $\mathrm{Pf}$ pertenece al anillo de invariantes $\RR[\mathfrak{so}(n)]^{\mathrm{SO}(n)}$.

\begin{thm}
  Si $n=2m+1$ es impar, entonces
  \begin{equation*}
\RR[\mathfrak{so}(n)]^{\mathrm{SO}(n)} = \RR[f_1,\dots,f_m].
  \end{equation*}
  Si $n=2m$ es par, entonces
  \begin{equation*}
\RR[\mathfrak{so}(n)]^{\mathrm{SO}(n)} = \RR[f_1,\dots,f_{m-1},\mathrm{Pf}].
  \end{equation*}
\end{thm}

\begin{proof}
Definimos $T\subset \mathrm{SO}(n)$ como el subgrupo de Lie conexo cuya álgebra de Lie $\mathfrak{t}$ está formada por las matrices de la forma
\begin{equation*} [x_1,\dots,x_m]:=
\begin{pmatrix}
0 & x_1 \\
-x_1 & 0 \\
 &  & \ddots \\
...
\end{pmatrix},
\end{equation*}
si $n=2m+1$ es impar. Toda matriz antisimétrica puede conjugarse por un elemento de $\mathrm{SO}(n)$ a una matriz de la forma anterior. Por tanto, se cumplen las hipótesis del teorema previo y tenemos que la restricción induce un isomorfismo
\begin{equation*}
\RR[\mathfrak{so}(n)]^{\mathrm{SO}(n)} \rightarrow \RR[\mathfrak{t}]^N,
\end{equation*}
para
\begin{equation*}
N = \left\{ A \in \mathrm{SO}(n): A \mathfrak{t} A^{-1} \subset \mathfrak{t} \right\}.
\end{equation*}

Las funciones $f_1,\dots,f_m$ evaluadas en un elemento $[x_1,\dots,x_m]\in \mathfrak{t}$ son las funciones simétricas elementales en las variables $(x_1)^2,\dots,(x_m)^2$, por tanto,
\begin{equation*}
\RR[f_1,\dots,f_m] = \RR[(x_1)^2,\dots,(x_m)^2]^{\mathfrak{S}_m}.
\end{equation*}
Ahora, para cada par $(i,j)$, con $1\leq i<j \leq m$ podemos encontrar una matriz $A\in N$ tal que
\begin{equation*}
A [x_1,\dots,x_i,\dots,x_j,\dots,x_m] A^{-1} = [x_1,\dots,x_j,\dots,x_i,\dots,x_m].
\end{equation*}
Por ejemplo, para $i=1$ y $j=2$ podemos tomar
\begin{equation*}
  A=
  \begin{pmatrix}
    0 & \mathbb{I}_2 \\
...
\end{pmatrix}.
\end{equation*}
También, para cada $i\in \left\{ 1,\dots,m \right\}$ podemos encontrar una matriz $B \in \mathrm{O}(n)$ con $B\mathfrak{t} B^{-1}\subset \mathfrak{t}$ tal que
\begin{equation*}
B [x_1,\dots,x_i,\dots,x_m] B^{-1} = [x_1,\dots,-x_i,\dots,x_m].
\end{equation*}
Por ejemplo, para $i=1$, podemos tomar
\begin{equation*}
  B =
  \begin{pmatrix}
    0 & 1 \\
...
\end{pmatrix}.
\end{equation*}
Además, si $n$ es impar, también podemos tomar, en vez de esta $B$, la matriz
\begin{equation*}
  B' =
  \begin{pmatrix}
    0 & 1 \\
...
\end{pmatrix},
\end{equation*}
que cumple las misma propiedades, y que además es un elemento de $\mathrm{SO}(n)$, y por tanto de $N$. Esto prueba una igualdad
\begin{equation*}
\RR[\mathfrak{t}]^N = \RR[(x_1)^2,\dots,(x_m)^2]^{\mathfrak{S}_m}=\RR[f_1,\dots,f_m],
\end{equation*}
para $n=2m+1$, y por tanto un isomorfismo
\begin{equation*}
\RR[\mathfrak{so}(2m+1)]^{\mathrm{SO}(2m+1)} \cong \RR[f_1,\dots,f_m].
\end{equation*}
Si $n$ es par, para cada par $(i,j)$, con $1\leq i<j \leq m$ podemos encontrar una matriz $C\in N$ tal que
\begin{equation*}
C [x_1,\dots,x_i,\dots,x_j,\dots,x_m] A^{-1} = [x_1,\dots,-x_i,\dots,-x_j,\dots,x_m].
\end{equation*}
Por ejemplo, para $i=1$ y $j=2$, podemos tomar
\begin{equation*}
  C =
...
\end{pmatrix}.
\end{equation*}
De aquí concluimos que todo elemento de $\RR[\ft]^N$ es una función simétrica en las variables $x_1,\dots,x_m$ y que cada uno de sus términos tiene la misma paridad en el grado en cada una de las variables. En otras palabras, todo elemento $f\in \RR[\ft]^N$ puede escribirse como
\begin{equation*}
f = g + x_1\cdots x_m h,
\end{equation*}
para $g,h \in \RR[(x_1)^2,\dots,(x_m)^2]$. Además, como $f$ es simétrica en las variables $x_1,\dots,x_m$, tenemos que $g,h\in \RR[(x_1)^2,\dots,(x_m)^2]^{\mathfrak{S}_m}=\RR[f_1,\dots,f_m]$. Finalmente, como $\mathrm{Pf}$ restringido a $\mathfrak{t}$ es igual al polinomio $x_1\cdots x_m$ y $\mathrm{Pf}^2=f_m$, concluimos que
\begin{equation*}
\RR[\ft]^N = \RR[f_1,\dots,f_{m-1},\mathrm{Pf}],
\end{equation*}
y por tanto tenemos un isomorfismo
\begin{equation*}
\RR[\mathfrak{so}(2m)]^{\mathrm{SO}(2m)} \cong \RR[f_1,\dots,f_{m-1},\mathrm{Pf}],
\end{equation*}
como queríamos probar.
\end{proof}

\section{El homomorfismo de Chern--Weil}
\subsection{Construcción} En esta sección, vamos a ver cómo, a cada $G$-fibrado principal $P\rightarrow M$ se le puede asociar un homomorfismo de $\RR$-álgebras
\begin{equation*}
w_P: \RR[\fg]^G \rightarrow H^*(M,\RR),
\end{equation*}
denominado el \emph{homomorfismo de Chern--Weil}.

Comenzamos fijando una conexión $A$ en el fibrado $P$, con curvatura $F_A\in \Omega^2(P;\fg)$. Ahora, para cada elemento homogéneo $f\in \RR[\fg]^G$, con $k=\deg f$, definimos la $2k$-forma $f(F_A)=f(F_A\wedge \overset{k}{\cdots} \wedge F_A)\in \Omega^{2k}(P)$ como
\begin{align*}
f(F_A)&(X_1,\dots,X_{2k}) \\
  &= \frac{1}{(2k)!} \sum_{\sigma \in \mathfrak{S}_{2k}} (-1)^\sigma \tilde{f}(F_A(X_{\sigma(1)},X_{\sigma(2)}),\dots,F_A(X_{\sigma(2k-1)},X_{\sigma(2k)})),
\end{align*}
para $X_1,\dots,X_{2k} \in \fX(P)$. Aquí, $\tilde{f}:\fg \times \overset{k}{\cdots} \times \fg \rightarrow \RR$ denota la \emph{polarización} de $f$; es decir, $\tilde{f}$ es la única aplicación $k$-lineal tal que $f(x)=\tilde{f}(x,\dots,x)$.

Recordemos que, equivalentemente, la curvatura puede verse como una $2$-forma en $M$ con valores en el fibrado adjunto $\ad P$, es decir $F_A \in \Omega^2(M;\ad P)$. Ahora, como $f$ es $\ad$-invariante, podemos definir la aplicación
\begin{equation*}
\bar{f}: \ad P \times \overset{k}{\cdots} \times \ad P \rightarrow \RR,\ ([(p,x_1)],\dots,[(p,x_k)]) \mapsto f(x_1,\dots,x_k).
\end{equation*}
Esto permite definir una $2$-forma $\bar{f}(F_A)=\bar{f}(F_A \wedge \overset{k}{\cdots} \wedge F_A)\in \Omega^2(M)$ como
\begin{equation*}
\bar{f}(F_A) = \frac{1}{2k!} \sum_{\sigma \in \mathfrak{S}_{2k}} (-1)^\sigma \bar{f}(F_A(X_{\sigma(1)},X_{\sigma(2)}),\dots,F_A(X_{\sigma(2k-1)},X_{\sigma(2k)})),
\end{equation*}
para $X_1,\dots,X_{2k} \in \fX(M)$. Por construcción, es claro que
\begin{equation*}
f(F_A) = \pi^* \bar{f}(F_A).
\end{equation*}

\begin{thm} La $2k$-forma $\bar{f}(F_A)$ es cerrada y su clase de cohomología de de Rham
        \begin{equation*}
w_P(f) := [\bar{f}(F_A)] \in H^{2k}(M,\RR)
        \end{equation*}
        es independiente de la conexión $A$.
\end{thm}

\begin{proof}
Para ver que $\bar{f}(F_A)$ es cerrada, basta ver que $f(F_A)$ lo es (ya que $\pi$ es una sumersión sobreyectiva). Ahora, por construcción $f(F_A)$ se anula en los vectores verticales, luego $df(F_A)=\pi_H^* df(F_A)$ y, por la identidad de Bianchi
\begin{equation*}
 df(F_A)=\pi_H^* df(F_A) = f(\pi_H^* d F_A \wedge F_A \wedge \overset{k}{\cdots} \wedge F_A) + \cdots + f( F_A \wedge \overset{k}{\cdots} \wedge F_A \wedge \pi_H^* dF_A) = 0.
\end{equation*}

Para ver que no depende de la conexión, de nuevo, basta ver que $f(F_A)$ no depende de la conexión. Tomemos dos conexiones distintas $A_0$ y $A_1$ en $P$ y consideremos su diferencia $\alpha=A_1-A_0\in \Omega^1(P;\fg)$. Podemos considerar la familia uniparamétrica de conexiones
\begin{equation*}
A_t = A_0 + t\alpha,
\end{equation*}
con curvaturas
\begin{equation*}
F_t = dA_t + A_t \wedge A_t = dA_0 + td\alpha + (A_0 + t\alpha) \wedge (A_0+t\alpha).
\end{equation*}
En particular,
\begin{equation*}
\frac{d}{dt} F_t = d\alpha + \alpha \wedge A_t + A_t \wedge \alpha = \pi_{H_{A_t}}^* d\alpha.
\end{equation*}
Queremos ver que $f(F_{A_1})-f(F_{A_0})=dh$ para cierta forma $h$, que queremos hallar. Vamos a construir esta $h$ como
\begin{equation*}
h = k \int_0^1 f(\alpha \wedge F_t \wedge \overset{k-1}{\cdots} \wedge F_t) dt.
\end{equation*}
En efecto,
\begin{align*}
dh &= k \int_0^1 df(\alpha \wedge F_t \wedge \overset{k-1}{\cdots} \wedge F_t) dt = k \int_0^1 \pi_{H_{A_t}}^* df(\alpha \wedge F_t \wedge \overset{k-1}{\cdots} \wedge F_t) dt  \\
&= k \int_0^1 f(\pi_{H_{A_t}}^*d\alpha \wedge F_t \wedge \overset{k-1}{\cdots} \wedge F_t) dt + k \int_0^1 f(\alpha \wedge \pi_{H_{A_t}}^* dF_t \wedge \overset{k-1}{\cdots} \wedge F_t) dt \\
& +k \int_0^1 f(\alpha \wedge F_t \wedge \overset{k-1}{\cdots} \wedge \pi_{H_{A_t}}^*dF_t) dt = k \int_0^1 f\left(\frac{dF_t}{dt} \wedge F_t \wedge \overset{k-1}{\cdots} \wedge F_t\right) dt  \\
  &= \int_0^1 \frac{d}{dt} f(F_t) dt = f(F_1) - f(F_0),
\end{align*}
como queríamos probar.
\end{proof}

\subsection{Clases características} Supongamos que $G$ es un grupo de Lie cuyo anillo de invariantes $\RR[\fg]^G$ es un anillo de polinomios generado por ciertos polinomios invariantes $f_1,\dots,f_r\in \RR[\fg]^G$; es decir,
\begin{equation*}
\RR[\fg]^G=\RR[f_1,\dots,f_r].
\end{equation*}
Este es el caso, por ejemplo, del grupo $\SO(n)$, como hemos visto en la sección anterior.

Dado un $G$-fibrado principal $P\rightarrow M$, sus \emph{clases características} son las clases de cohomología
\begin{equation*}
w_P(f_i) \in H^*(M,\RR).
\end{equation*}

Por ejemplo, para $G=\OO(n)$, las clases características de un $\OO(n)$-fibrado principal $P\rightarrow M$ de la forma
\begin{equation*}
p_i(P)=w_P(f_i) \in H^*(M,\RR)
\end{equation*}
se denominan las \emph{clases de Pontrjagin} de $P$. Además, si $n=2m$ es par, y el fibrado $P$ admite una reducción a $\SO(2m)$, entonces podemos considerar la \emph{clase de Euler} de $P$, definida como la imagen del pfaffiano $\mathrm{Pf}\in \RR[\so(2m)]^{\SO(2m)}$ por el homomorfismo de Chern--Weil,
\begin{equation*}
e(P)= w_P(\mathrm{Pf}) \in H^{2m}(M,\RR).
\end{equation*}

Es particularmente interesante el caso en el que $\pi:P=\mathcal{B}_{\OO}(M)\rightarrow M$ es el fibrado de referencias de una variedad riemanniana $(M,g)$, equipado con la conexión de Levi--Civita, que denotamos por $A$. En ese caso, se definen las \emph{clases de Pontrjagin de $M$} como
\begin{equation*}
p_i(M) = p_i(\mathcal{B}_{\OO}(M)),
\end{equation*}
que están representadas por las formas $\gamma_i \in \Omega(M)$ que cumplen
\begin{equation*}
\pi^* \gamma_i = f_i(F_A).
\end{equation*}
Además, si $M$ es orientable y de dimensión par $n=2m$ y fijamos una orientación en $M$, obtenemos una reducción del grupo de estructura de $P$ a $\SO(2m)$ y podemos definir la \emph{clase de Euler de $M$} como
\begin{equation*}
e(M) = e(\mathcal{B}_{\SO}(M)),
\end{equation*}
que está representada por la forma $\gamma \in \Omega^{2m}(M)$ que cumple
\begin{equation*}
\pi^* \gamma= \mathrm{Pf}(F_A).
\end{equation*}

\section{El teorema de Chern--Gauss--Bonnet}
\begin{thm}[Teorema de Chern--Gauss--Bonnet]
  Sea $(M,g)$ una variedad riemanniana orientada de dimensión par $n=2m$ y sea $e(M)=[\gamma]$ su clase de Euler, que está representada por una forma $\gamma \in \Omega^{2m}(M)$ que cumple
\begin{equation*}
\pi^* \gamma= \mathrm{Pf}(F_A),
\end{equation*}
donde $A$ es la conexión de Levi--Civita de $(M,g)$ (vista como una conexión en el fibrado de referencias $\pi: \mathcal{B}_{\SO}(M)\rightarrow M$).
  Se tiene que
  \begin{equation*}
\int_{M} \gamma = (2\pi)^m \chi(M),
  \end{equation*}
donde $\chi(M)$ es la característica de Euler de $M$.
\end{thm}

Antes de demostrar el teorema, necesitamos un lema previo.

\begin{lemma}
  Sea
  \begin{equation*}
SM = \left\{ (x,X_x) \in TM : \langle X_x,X_x \rangle_x = 1 \right\},
  \end{equation*}
  con $p:SM\rightarrow M$, $(x,X_x)\mapsto x$ el \emph{fibrado de esferas tangente} a $M$. Existe una $(2m-1)$-forma $\Pi \in \Omega^{2m-1}(SM)$ tal que \[d\Pi = p^* \gamma\] y tal que, para cada $x\in M$, la integral \[C_m:=\int_{p^{-1}(x)} \Pi\]
  es una constante que sólo depende de $m$.
\end{lemma}


\begin{proof}[Demostración del lema]
  Podemos ver el fibrado de referencias $\cB_{\SO}(M)$ como un $\SO(2m-1)$-fibrado principal sobre $SM$ por medio de la aplicación
  \begin{equation*}
\cB_{\SO}(M) \rightarrow SM,\ (e_1,\dots,e_{2m}) \mapsto e_1.
  \end{equation*}
  Por otra parte, podemos considerar el $\SO(2m)$-fibrado principal $p^*\cB_{\SO}(M)\rightarrow SM$, que es simplemente la extensión del fibrado anterior por la inclusión natural $\SO(2m-1)\hookrightarrow \SO(2m)$.

  A partir de la conexión de Levi--Civita $A$ en $\cB_{\SO}(M)\rightarrow M$ podemos definir dos conexiones $A_0$ y $A_1$ en $p^* \cB_{\SO}(M)\rightarrow SM$. En primer lugar, definimos $A_1$ simplemente como el pullback $A_0=p^*A$ de la conexión de Levi--Civita. Ahora, podemos proyectar $A$ a la subálgebra $\mathfrak{so}(2m-1)\subset \mathfrak{so}(2m)$, lo que determina una conexión en el $\SO(2m-1)$-fibrado $\cB_{\SO}(M)\rightarrow SM$ y, a su vez, esta conexión se puede extender al fibrado $p^*\cB_{\SO}(M)\rightarrow SM$, lo que determina la otra conexión $A_0$. La diferencia entre estas dos conexiones es una forma
  \begin{equation*}
\alpha:= A_1 - A_0 \in \Omega^1(p^* \cB_{\SO}(M),\mathfrak{so}(2m)).
  \end{equation*}
  Ahora, para cada $t\in [0,1]$, definimos la conexión $A_t = A_0 +t \alpha$, y denotamos $F_t=F_{A_t}$. Finalmente, ponemos
  \begin{equation*}
\Pi = -m \int_0^1 \overline{\mathrm{Pf}}(\alpha \wedge F_t \wedge \overset{m-1}{\cdots}\wedge F_t) dt \in \Omega^{2m-1}(SM).
  \end{equation*}
  Un cálculo totalmente análogo al de la prueba de \todo{REF} muestra que
\begin{equation*}
d\Pi = \overline{\mathrm{Pf}}(F_1)- \overline{\mathrm{Pf}}(F_0).
\end{equation*}
Ahora, como $A_1 = p^* A$, tenemos que $F_1 = p^* F_A$, de modo que
\begin{equation*}
\overline{\mathrm{Pf}}(F_1) = p^* \overline{\mathrm{Pf}}(F_A) = p^* \gamma.
\end{equation*}
Por otra parte, como la restricción del pfaffiano a $\mathrm{so}(2m-1)$ es $0$, se tiene que
\begin{equation*}
\overline{\mathrm{Pf}}(F_0) = 0,
\end{equation*}
con lo cual $d\Pi = p^*\gamma$. Como la expresión de $\Pi$ en cada fibra es una expresión universal que sólo depende de la forma de Maurer--Cartan y las fibras de $SM$ se pueden identificar con la esfera unidad estándar $S^{2m-1}$, la integral $\int_{p^{-1}(x)}\Pi$ en cada fibra es una constante que tan sólo depende de $m$.
\end{proof}

\begin{proof}[Demostración del teorema]
  Sea $X\in \fX(M)$ un campo vectorial que sólo tiene ceros aislados. Este campo determina una sección $\bar{X}:M\rightarrow SM$ de $p$, poniendo \[\bar{X}_x=X_x / \lVert X_x \rVert_x,\] que está bien definida en el abierto $M'=M\setminus \left\{ \text{ceros de } X \right\}$.
  La imagen $N=\bar{X}(M')\subset SM$ es una subvariedad de $SM$ con borde. Además, su borde $\partial N$ es difeomorfo, como variedad orientada, a una unión disjunta
  \begin{equation*}
\partial N \cong \coprod_{x\in \left\{ \text{ceros de } X \right\}} (p^{-1}(x))^{\sqcup \ind_x X};
  \end{equation*}
  aquí, el signo de $\ind_x X$ indica la orientación que se le pone a $p^{-1}(x)$.
  Por tanto, por el teorema de Stokes
  \begin{equation*}
\int_M \gamma = \int_N p^*\gamma = \int_N d\Pi = \int_{\partial N} \Pi = \sum_{x\in \left\{ \text{ceros de } X \right\}} \ind_x X \int_{S_x}p^{-1}(x) = C_m \ind X.
  \end{equation*}
Por el teorema de Poincaré--Hopf, $\ind X = \chi(M)$, y concluimos que
\begin{equation*}
\int_M \gamma = C_m \chi(M).
\end{equation*}
Basta considerar ahora el caso en que $M=(S^2)^m$. El término de la izquierda es
\begin{equation*}
\int_M \gamma = (\text{Área}(S^2))^m = (4\pi)^m = (2\pi)^m (\chi(S^2))^m = (2\pi)^m \chi((S^2)^m),
\end{equation*}
con lo cual
\begin{equation*}
C_m = (2\pi)^m,
...
\chapter{Un poco de análisis global: la ecuación de Poisson}
\section{La ecuación de Poisson en variedades compactas}
\subsection{} Durante todo este capítulo, $M$ denota una variedad diferenciable conexa, compacta, orientada y sin borde, de dimensión $n\geq 2$. Fijamos, además, una métrica riemanniana $g$ en $M$, que en coordenadas $\bm{x}=(x^1,\dots,x^n)$ se escribe como
\begin{equation*}
g = g_{ij} dx^i dx^j.
\end{equation*}
En particular, la forma de volúmen se escribe en coordenadas como
\begin{equation*}
\mathrm{dvol}_g = V dx^1\wedge \cdots \wedge dx^n,
\end{equation*}
para $V=\sqrt{\det g}$.

\subsection{Gradiente, divergencia y laplaciano} Dada una función diferenciable $f:M\rightarrow \RR$, se define su \emph{gradiente} (con respecto a $g$)
\begin{equation*}
\grad_g f = (df)^\sharp \in \fX(M)
\end{equation*}
como el campo vectorial dual a la $1$-forma $df\in \Omega^1(M)$ con respecto a la métrica riemanniana. En coordenadas,
\begin{equation*}
(\grad_g f)^j = g^{ij} \partial_if.
\end{equation*}
En particular, si $g^{ij}=\delta^{ij}$ está representada por la matriz identidad, recuperamos la expresión típica del gradiente
\begin{equation*}
(\grad_g f)^i = \partial_if .
\end{equation*}


Dado un campo vectorial $X\in \fX(M)$, su \emph{divergencia} $\Div_g X: M\rightarrow \RR$ se define por medio de la ecuación
\begin{equation*}
\mathcal{L}_X \mathrm{dvol}_g = (\Div_g X) \mathrm{dvol}_g.
\end{equation*}
Ahora, por la fórmula mágica de Cartan,
\begin{equation*}
\mathcal{L}_X \mathrm{dvol}_g = d(i_X\mathrm{dvol}_g) + i_X(d\mathrm{dvol}_g) = d(i_X\mathrm{dvol}_g).
\end{equation*}
Por tanto, equivalentemente,
\begin{equation*}
(\Div_g X) \mathrm{dvol}_g=d(i_X\mathrm{dvol}_g).
\end{equation*}
En coordenadas, para un campo $X=X^i \partial_i$,
\begin{equation*}
\Div_g X = \partial_i X^i + X^i \left( \frac{1}{2} g^{kl} \partial_{i}g_{kl} \right).
\end{equation*}
En particular, si $g^{ij}=\delta^{ij}$,
\begin{equation*}
\Div_g X = \sum_{i} \partial_i X^i.
\end{equation*}
Del teorema de Stokes se sigue inmediatamente una ``versión riemanniana'' del teorema de la divergencia:
\begin{equation*}
\int_{N} (\Div_g X) \mathrm{dvol}_g = \int_{\partial N} i_X \mathrm{dvol}_g.
\end{equation*}


Finalemente, al igual que en el espacio euclidiano, definimos el \emph{laplaciano} de una función $f:M\rightarrow \RR$ como la divergencia de su gradiente:
\begin{equation*}
\Delta_g f = \Div_g (\grad_g f)
\end{equation*}
El operador $\Delta_g$ se denomina el \emph{operador de Laplace--Beltrami}.
En coordenadas,
\begin{equation*}
\Delta_g f = g^{ij} \frac{\partial^2 f}{\partial x^i \partial x^j} - g^{il} \Gamma^j_{il} \partial_j f.
\end{equation*}
En particular, si $g^{ij}=\delta^{ij}$,
\begin{equation*}
\Delta_g f = \sum_{i} \frac{\partial^2 f}{(\partial x^i)^2}.
\end{equation*}

\subsection{} La \emph{ecuación de Poisson} en $(M,g)$, para una función diferenciable $\rho:M\rightarrow \RR$, es
\begin{equation*}
\Delta_g \phi = \rho.
\end{equation*}
Aquí, $\phi$ es la incógnita, es decir, se busca una función $\phi$ cuyo laplaciano sea $\rho$.

Existe una primera condición necesaria para la existencia de una tal $\phi$, que es que
\begin{equation*}
\int_M \rho\ \mathrm{dvol}_g = 0.
\end{equation*}
En efecto, ya que
\begin{equation*}
\rho\ \mathrm{dvol}_g = \Div_g (\grad_g f)\ \mathrm{dvol}_g = d(i_{\grad_g f}\ \mathrm{dvol}_g),
\end{equation*}
y $\partial M = \varnothing$.

\begin{thm}
Para toda función diferenciable $\rho:M\rightarrow \RR$ con $\int_M \rho\ \mathrm{dvol}_g = 0$, existe una función diferenciable $\phi:M\rightarrow \RR$ tal que
\begin{equation*}
\Delta_g \phi = \rho.
\end{equation*}
Además, esta función es única salvo por la adición de una constante.
\end{thm}

Dedicamos el resto del capítulo a la demostración de este teorema.

\section{Esbozo de la demostración}
\subsection{} La métrica riemanniana nos permite definir un producto escalar en el espacio de las funciones diferenciables $f:M\rightarrow \RR$, poniendo
\begin{equation*}
\langle f_1, f_2 \rangle_{L^2} = \int_M f_1f_2\ \mathrm{dvol}_g.
\end{equation*}
La completación del espacio $C^\infty(M)$ con respecto a la métrica determinada por este producto escalar es un espacio de Hilbert que se denota por $L^2(M)$.

También podemos definir un producto escalar en el espacio de campos $\fX(M)$, poniendo
\begin{equation*}
\langle X_1, X_2 \rangle_{L^2} = \int_M \langle X_{1,x}, X_{2,x} \rangle_x\ \mathrm{dvol}_g.
\end{equation*}
Esto nos permite definir el \emph{producto de Sobolev} en $C^\infty(M)$, como
\begin{equation*}
\langle f_1,f_2 \rangle_{W^{1,2}} = \langle f_1,f_2 \rangle_{L^2} + \langle \grad f_1, \grad f_2 \rangle_{L^2}.
\end{equation*}
La completación del espacio $C^\infty(M)$ con respecto a la métrica determinada por este producto escalar es un espacio de Hilbert que se denota por $W^{1,2}(M)$ y se denomina el \emph{espacio de Sobolev} de $M$.
Definimos también el \emph{producto de Dirichlet}, para $f_1,f_2 \in C^\infty(M)$,
\begin{equation*}
\langle f_1,f_2 \rangle_{H} = \langle \grad f_1, \grad f_2 \rangle_{L^2}.
\end{equation*}
El producto de Dirichlet no determina un producto escalar en $C^\infty(M)$, pero sí en el subespacio $C^\infty_0(M)\subset C^\infty(M)$ formado por las funciones diferenciables $f:M\rightarrow \RR$ con $\int_M f\ \dvol_g = 0$. Denotamos por $H$ al espacio de Hilbert obtenido como la completación de $C^\infty_0(M)$ con respecto al producto de Dirichlet. Necesitamos ahora la siguiente estimación fundamental, que probamos en la siguiente sección.

\begin{thm}[Desigualdad de Poincaré] \label{thm:desigualdad_poincare}
  Existe una constante $C>0$ tal que, para toda función $f\in C^\infty(M)$, se tiene
  \begin{equation*}
\lVert f - \bar{f}_M \rVert_{L^2} \leq C \lVert \grad_g f \rVert_{L^2},
  \end{equation*}
  donde $\bar{f}_M= \tfrac{1}{\mathrm{vol}(M)} \int_M f \dvol_g$. En particular, si $f \in C^\infty_0(M)$, se tiene que
  \begin{equation*}
\lVert f \rVert_{L^2} \leq C \lVert \grad_g f \rVert_{L^2}.
  \end{equation*}
\end{thm}

\subsection{} Como consecuencia de la desigualdad de Poincaré, la norma de Sobolev es equivalente a la determinada por el producto de Dirichlet en el espacio $C^\infty_0(M)$; por tanto el espacio de Hilbert $H$ es isométrico a la completación de $C^\infty_0(M)$ con respecto al producto de Sobolev. La clave es usar ahora el siguiente resultado fundamental de análisis funcional \todo{CITE}.


\begin{thm}[Teorema de representación de Riesz]
Sea $H$ un espacio de Hilbert. Para toda forma lineal acotada $\alpha:H\rightarrow \RR$ existe un único elemento $a\in H$ tal que
\begin{equation*}
\alpha(v) = \langle a, v\rangle_H,
\end{equation*}
para todo $v\in H$.
\end{thm}

\subsection{} Nótese ahora que una función $\phi \in C^\infty(M)$ satisface la ecuación $\Delta_g \phi = \rho$ si y sólo si, para toda función $f\in C^\infty(M)$ se cumple que
\begin{equation*}
\langle f,\phi \rangle_{H} = \langle f, \rho \rangle_{L^2}.
\end{equation*}
En efecto,
\begin{align*}
\langle \grad_g f, \grad_g \phi \rangle_{L^2} &= \int_{M}\langle (\grad_g f)_x, (\grad_g \phi)_x \rangle_x \dvol_g = \int_{M} df \wedge i_{\grad_g \phi} \dvol_g \\ &= \int_M f d(i_{\grad_g \phi} \dvol_g) = \int_M f \Delta_g \phi\ \dvol_g= \langle f, \rho \rangle_{L^2}.
\end{align*}

Basta entonces considerar la forma lineal
\begin{equation*}
\alpha_\rho: H \rightarrow \RR,\ f \mapsto \langle f, \rho \rangle_{L^2},
\end{equation*}
que es continua porque $H\subset L^2(M)$. Aplicando el teorema de representación de Riesz a esta forma lineal, encontramos un elemento $\phi\in H$ tal que
\begin{equation*}
\langle f, \phi \rangle_H = \langle f, \rho \rangle_{L^2},
\end{equation*}
para toda $f\in C^\infty_0(M)$. Este elemento, que llamaremos la \emph{solución débil} a la ecuación de Poisson es nuestro candidato a solución. Tan sólo necesitamos probar que, en efecto $\phi \in C^\infty_0(M)$; es decir, que $\phi$ es una función $C^\infty$. Como esta propiedad es local, basta demostrar el siguiente resultado.

\begin{thm}[Teorema de regularidad]
  Sea $\Omega\subset \RR^n$ un subconjunto acotado y sea $\rho \in C^\infty(M)$. Supongamos que $\phi$ es una función $L^2$ en $\Omega$ con la propiedad de que, para toda función $f\in C^\infty(\Omega)$ con soporte compacto, se tiene que
  \begin{equation*}
\int_\Omega \Delta f \phi = \int_\Omega f \rho.
  \end{equation*}
  Entonces $\phi$ es $C^\infty$ y satisface la ecuación $\Delta \phi = \rho$.
\end{thm}

\section{La desigualdad de Poincaré}
El primer paso para demostrar la desigualdad de Poincaré es demostrar un resultado similar en el espacio euclidiano.

\begin{lemma} \label{lema:poincare-euclidiano}
Sea $\Omega \subset \RR^n$ un subconjunto abierto, acotado y convexo, con diámetro $d$. Entonces, para toda función diferenciable $f: \bar{\Omega}\rightarrow \RR$, se tiene que
\begin{equation*}
\int_{\Omega} \lvert f - \bar{f}_{\Omega} \rvert^2 \leq 2^{n-1} d^2 \int_\Omega \lVert \nabla f \rVert^2,
\end{equation*}
donde $\bar{f}_{\Omega} = \tfrac{1}{\mathrm{vol}(\Omega)} \int_{\Omega} f$.
\end{lemma}

\subsection{} Antes de hacer la demostración, recordemos el siguiente hecho clave, válido para cualquier espacio de medida finita $(A,\mu)$, con $\mu(A)=m>0$: para toda función $f\in L^1(A,\mu)$,
\begin{equation} \label{eq:promedios}
\int_A \lvert f - \bar{f}_{A} \rvert^2 d\mu = \frac{1}{2m} \int_A \int_A \lvert f(x)- f(y) \lvert^2 d\mu(x) d\mu(y),
\end{equation}
donde $\bar{f}_{A} = \frac{1}{m} \int_A f$. Esto se sigue de hecho de que
\begin{equation} \label{eq:promedios2}
\int_A \lvert f - \bar{f}_A \rvert^2 d\mu \leq \int_A \lvert f - c \vert^2 d\mu
\end{equation}
para cualquier constante $c$.

\begin{proof}[Demostración del Lema \ref{lema:poincare-euclidiano}]
  Como $\Omega$ es convexo, para cualesquiera $x,y \in \Omega$, el segmento que los une está en $\Omega$, de modo que
  \begin{equation*}
f(y)-f(x) = \int_0^1 \langle \nabla f((1-t)x + ty), y-x \rangle dt.
  \end{equation*}
Como $\lvert y-x \rvert \leq d$, aplicando la desigualdad de Cauchy--Schwarz,
\begin{equation*}
\lvert f(y)-f(x) \vert^2 \leq d^2 \int_0^1 \lVert \nabla f((1-t)x + ty) \rVert^2 dt.
\end{equation*}
Integrando en $(x,y)$ y usando el teorema de Fubini,
\begin{equation*}
\int_{\Omega \times \Omega} \lvert f(y)-f(x) \vert^2 dx dy \leq d^2 \int_0^1 \int_{\Omega \times \Omega} \lVert \nabla f((1-t)x + ty) \rVert^2 dx dy dt.
\end{equation*}
Separamos ahora esta integral como
\begin{align*}
\int_0^1 \int_{\Omega \times \Omega} \lVert \nabla f((1-t)x + ty) \rVert^2 dx dy dt = & \int_0^{1/2} \int_{\Omega \times \Omega} \lVert \nabla f((1-t)x + ty) \rVert^2 dx dy dt \\ & + \int_{1/2}^1 \int_{\Omega \times \Omega} \lVert \nabla f((1-t)x + ty) \rVert^2 dx dy dt
\end{align*}
y acotamos cada uno de los términos por separado.
Para $0\leq t \leq 1/2$, dejamos fijo $y$ y hacemos un cambio de variable $w=(1-t)x + ty$. Como $\Omega$ es convexo, tenemos que $w\in \Omega$ para estos valores de $t$, y $dw = (1-t)^n dx$. Como $(1-t)^{-n}\leq 2^n$, concluimos que
\begin{equation*}
\int_{\Omega} \left( \int_\Omega \lVert \nabla f((1-t)x + ty) \rVert^2 dx  \right) dy \leq 2^n \mathrm{vol}(\Omega) \int_{\Omega} \lVert \nabla f(w) \rVert^2 dw.
\end{equation*}
Para $1/2\leq t \leq 1$ obtenemos la misma cota dejando fijo $x$ y poniendo $w=(1-t)x + ty$, de modo que $dw = t^n dy$, con $t^{-n}\leq 2^n$. Concluimos que
\begin{equation*}
\int_{\Omega \times \Omega} \lvert f(y)-f(x) \vert^2 dx dy \leq d^22^n \mathrm{vol}(\Omega) \int_{\Omega} \lVert \nabla f(w) \rVert^2 dw.
\end{equation*}
El resultado se obtiene de la igualdad \eqref{eq:promedios}.
\end{proof}

Fijemos ahora un punto $x\in M$ y un sistema de coordenadas $(U',\bm{x})$ en un entorno de $x$ con $\bm{x}(x)=0$ y $g_{ij}(0)=\delta_{ij}$, para $i,j=1,\dots,n$. Por continuidad de las $g_{ij}$, existe un $r>0$ tal que la bola $B(0,2r)\subset \bm{x}(U')$ y de forma que, para todo $v \in \overline{B(0,2r)}$, se tiene que
\begin{equation*}
\frac{1}{2} \lVert v \rVert \leq \lVert v \rVert_g \leq 2\lVert v \rVert.
\end{equation*}
Denotamos $U_x=\bm{x}^{-1}(B(0,r))$.

\begin{lemma} \label{lema:poincare-local}
  Para toda $f\in C^\infty(M)$, se tiene que
  \begin{equation*}
\int_{U_x} \lvert f - \bar{f}_{U_x} \rvert^2 \dvol_g \leq 2^{2n+2} r^2 \int_{U_x} \lVert\grad_g f \rVert_g^2 \dvol_g.
  \end{equation*}
\end{lemma}

\begin{proof}
Usando las cotas de $\lVert v \rVert_g$ para $v\in \overline{B(0,2r)}$, tenemos que
\begin{equation*}
\dvol_g = V dx^1\wedge \cdots \wedge dx^n,
\end{equation*}
con $V:\overline{B(0,2r)}\rightarrow \RR$ una función con $2^{-n/2}\leq V \leq 2^{n/2}$.
Ahora, la función $\tilde{f}=f\circ \bm{x}^{-1}:\overline{B(0,r)}\rightarrow \RR$ es una función diferenciable, y usando de nuevo las cotas para $\lVert - \rVert_g$,
\begin{equation*}
\lVert \grad_g f \rVert_g^2 = g^{ij} \partial_i \tilde{f} \partial_j\tilde{f} \geq \frac{1}{2} \lVert \nabla \tilde{f} \rVert^2.
\end{equation*}
Si denotamos $c=\frac{1}{\mathrm{vol}(B(0,r))} \int_B \tilde{f}$, usando la desigualdad \eqref{eq:promedios2}, tenemos que
\begin{equation*}
\int_{U_x} \lvert f - \bar{f}_{U_x} \rvert^2 \dvol_g \leq \int_{U_x} \lvert f - c \rvert^2 \dvol_g.
\end{equation*}
Finalmente, usando las cotas anteriores y el Lema \ref{lema:poincare-euclidiano} aplicado a $\Omega=B(0,r)$, que es convexo, con diámetro $d=2r$, concluimos que
\begin{align*}
  \int_{U_x} \lvert f - \bar{f}_{U_x} \rvert^2 \dvol_g &\leq \int_{U_x} \lvert f - c \rvert^2 \dvol_g \leq 2^{n/2} \int_{B(0,r)} \lvert \tilde{f} - c \rvert^2 \leq 2^{n/2}2^{n-1}(2r)^2 \int_{B(0,r)} \lVert \nabla \tilde{f} \rVert^2 \\
                                & \leq 2^{n/2}2^{n+1}r^2 2 \int_{B(0,r)} \lVert \grad_g f \rVert_g^2 \leq 2^{n/2} 2^{n+2} r^2 2^{n/2} \int_{U_x} \lVert \grad_g f \rVert_g^2 \dvol_g \\
                                  &= 2^{2n+2} r^2 \int_{U_x}\lVert \grad_g f \rVert_g^2 \dvol_g,
\end{align*}
como queríamos probar.
\end{proof}

El siguiente paso es ver qué sucede en las intersecciones. Para ello, decimos que un conjunto $U\subset M$ \emph{satisface $P(C_U)$} si
\begin{equation*}
\int_U \lvert f- \bar{f}_U \rvert^2 \dvol_g \leq C_U \int_U \lVert \grad_g f \rVert^2 \dvol_g
\end{equation*}
para toda $f\in C^\infty(M)$.

\begin{lemma}
  Sean $U,V\subset M$ conjuntos abiertos con $\mathrm{vol}(U\cap V)>0$, y supongamos que $U$ satisface $P(C_U)$ y $V$, $P(C_V)$. Entonces, $U\cup V$ satisface $P(C)$, con
  \begin{equation*}
C= 2 \left( 2 + \frac{4\mathrm{vol}(V)}{\mathrm{vol}(U\cap V)} \right) \max(C_U,C_V).
  \end{equation*}
\end{lemma}

\begin{proof}
  En primer lugar, comparamos los promedios en la intersección $U\cap V$,
  \begin{align*}
    \lvert \bar{f}_U - \bar{f}_V \rvert^2 &= \frac{1}{\mathrm{vol}(U\cap V)} \int_{U\cap V} \lvert \bar{f}_U - \bar{f}_V \rvert^2 \dvol_g \\ & \leq \frac{2}{\mathrm{vol}(U\cap V)} \int_{U\cap V} (\lvert f- \bar{f}_U\rvert^2 + \lvert f - \bar{f}_V \rvert^2) \dvol_g \\
    & \leq \frac{2}{\mathrm{vol}(U\cap V)}\left(\int_{U} \lvert f- \bar{f}_U\rvert^2 + \int_V\lvert f- \bar{f}_V\rvert^2 \right).
  \end{align*}
  Ahora,
  \begin{align*}
    \int_{U\cup V} &\lvert f - \bar{f}_U \rvert^2 \dvol_g \leq \int_{U} \lvert f - \bar{f}_U \rvert^2 \dvol_g + \int_{V} \lvert f - \bar{f}_U \rvert^2 \dvol_g \\
    & \leq \int_{U} \lvert f - \bar{f}_U \rvert^2 \dvol_g + 2 \int_{V} \lvert f - \bar{f}_V \rvert^2 \dvol_g + 2\mathrm{vol}(V)     \lvert \bar{f}_U - \bar{f}_V \rvert^2 \\
                                         & \leq \left( 2 + \frac{4\mathrm{vol}(V)}{\mathrm{vol}(U\cap V)} \right)\int_{U} \lvert f - \bar{f}_U \rvert^2 \dvol_g + \int_{V} \lvert f - \bar{f}_V \rvert^2 \dvol_g\\
    & \leq \left( 2 + \frac{4\mathrm{vol}(V)}{\mathrm{vol}(U\cap V)} \right) \max(C_U,C_V) \left( \int_U \lVert \grad_g f \rVert^2 \dvol_g +  \int_V \lVert \grad_g f \rVert^2 \dvol_g\right).
  \end{align*}
Finalmente, $\int_U \lVert \grad_g f \rVert^2 \dvol_g +  \int_V \lVert \grad_g f \rVert^2 \dvol_g \leq 2\int_{U\cup V} \lVert \grad_g f \rVert^2 \dvol_g$ y, usando la desigualdad \eqref{eq:promedios2}, tenemos que $\int_{U\cup V} \lvert f - \bar{f}_U \rvert^2 \dvol_g \leq \int_{U\cup V} \lvert f - \bar{f}_U \rvert^2 \dvol_g$.
\end{proof}

La familia de subconjuntos $\left\{ U_x:x\in M  \right\}$ obtenida del Lema \ref{lema:poincare-local} forma un recubrimiento abierto de $M$ y por compacidad podemos obtener un subrecubrimiento finito $U_1,\dots,U_N$, cada uno de los cuales satisface $P(C_i)$, con $C_i=2^{2n+2} r_i^2$. Además, como $M$ es conexa, estos abiertos cumplen que, para todo $k=1,\dots,N$, la intersección $U_k \cap \bigcup_{i=1, i\neq k}^N U_i \neq \varnothing$. Es más, podemos reordenar estos abiertos de tal forma que, si definimos, para cada $k=2,\dots,N$, el abierto $W_k=U_1\cup \cdots \cup U_{k-1}$, entonces las intersecciones $U_k \cap W_k \neq \varnothing$.

\begin{proof}[Demostración de \ref{thm:desigualdad_poincare}]
Sabemos que $W_1=U_1$ satisface $P(C_1)$ y, si $W_{k-1}$ satisface $P(D_{k-1})$, entonces, como $W_{k-1}\cap U_k$ es un abierto no vacío, la unión $W_k$ satisface $P(D_k)$, con
\begin{equation*}
D_k = 2 \left( 2+ \frac{4\mathrm{vol}(U_k)}{\mathrm{vol}(W_{k-1}\cap U_k)} \right) \max(D_{k-1},C_k).
\end{equation*}
Por tanto, $M=W_N$ satisface $P(D_N)$.
\end{proof}


\section{Regularidad de las funciones armónicas}
\subsection{} Para demostrar el teorema de regularidad, el primer paso es darse cuenta de que podemos reducir al caso en que $\rho=0$. En efecto, como la regularidad es una propiedad local, basta probar que $\phi$ es $C^\infty$ en cualquier subconjunto $\Omega'\subset \Omega$ cuya adherencia está contenida en el interior de $\Omega$. Podemos entonces tomar una función $\rho'$ que sea igual a $\rho$ en un entorno de la adherencia de $\Omega'$ y que tenga soporte compacto en $\Omega$. Supongamos ahora que podemos hallar una solución $\phi'\in C^\infty(\Omega)$ de la ecuación $\Delta \phi'=\rho'$. Entonces $\psi=\phi-\phi'$ es una solución débil de la ecuación $\Delta \psi=0$ en $\Omega'$. Si probamos que $\psi$ es $C^\infty$, entonces también lo será $\phi$. Una función con $\Delta \psi = 0$ se denomina una \emph{función armónica}.

\subsection{} En primer lugar, veamos que, en efecto, es posible hallar una solución $\phi'\in C^\infty(\Omega)$ de la ecuación $\Delta \phi' = \rho'$, donde $\rho'$ tiene soporte compacto en $\Omega$. Esta solución se conoce clásicamente, y se construye en términos del \emph{potencial newtoniano}
\begin{equation*}
  \Gamma(x) =
  \begin{cases}
\frac{1}{(n-2) \mathrm{vol}(S^{n-1})} \frac{1}{\lVert x \rVert^{n-2}},& n\geq 3, \\
- \frac{1}{2\pi} \log \lVert x \rVert,& n=2.
\end{cases}
\end{equation*}
Esta función no está definida para $x=0$, pero es una función localmente integrable en $\RR^n$ y, para toda función $f\in C^\infty(\RR^n)$ con soporte compacto, la convolución $\Gamma * f$, definida como
\begin{equation*}
\Gamma * f (x) = \int_{\RR^n} \Gamma(y) f(x-y) dy,
\end{equation*}
pertenece a $C^\infty(\RR^n)$. Usando el siguiente lema, podemos tomar como nuestra solución $\phi'$ la función
\begin{equation*}
\phi' = \Gamma * \rho'.
\end{equation*}
...
\begin{lemma}
  Si $f\in C^\infty(\RR^n)$ es una función con soporte compacto, entonces se tiene
  \begin{equation*}
\Gamma*(\Delta f) = f
  \end{equation*}
  y
...
\begin{proof}
  Para la primera igualdad, basta calcular en el punto $x=0$. Tenemos entonces
  \begin{equation*}
(\Gamma * \Delta f) (0) = \int_{\RR^n} \Gamma(y) \Delta f (y) dy.
  \end{equation*}
  Para calcular esta integral, podemos calcularla en un conjunto $\Omega_{\varepsilon,R}=B(0,R)\setminus B(0,\varepsilon)$ y hacer tender $R \rightarrow \infty$ y $\varepsilon \rightarrow 0$. Tenemos entonces, por el teorema de la divergencia
  \begin{equation*}
\int_{\Omega_{\varepsilon,R}} \Gamma(y)\Delta f(y) dy = \int_{\Omega_{\varepsilon,R}} \Delta \Gamma(y) f(y) dy + \int_{\partial \Omega} (-\Gamma \partial_rf + f \partial_r \Gamma) dS,
  \end{equation*}
  donde $dS$ es el elemento de superficie en $\partial \Omega$ y $r=\lVert x \rVert$.
  Ahora, en $\RR^n\setminus \left\{ 0 \right\}$, tenemos
  \begin{equation*}
\frac{\partial \Gamma}{\partial r} = - \frac{1}{\mathrm{vol}(S^{n-1}) r^{n-1}}
  \end{equation*}
  y
  \begin{equation*}
\Delta \Gamma = \frac{\partial^2 \Gamma}{\partial r^2} + \frac{n-1}{r} \frac{\partial \Gamma}{\partial r} = 0.
  \end{equation*}
  Por tanto,
  \begin{equation*}
\int_{\Omega_{\varepsilon,R}} \Gamma(y)\Delta f(y) dy = \int_{r=\varepsilon} (-\Gamma \partial_rf + f \partial_r \Gamma) dS + \int_{r=R} (-\Gamma \partial_rf + f \partial_r \Gamma) dS.
  \end{equation*}
  Como $f$ tiene soporte compacto, la segunda integral en el lado derecho se anula si tomamos $R$ suficientemente grande. Por otra parte,
  \begin{equation*}
\left \lvert \int_{r=\varepsilon}\Gamma \partial_rf dS \right\rvert \leq \lvert \Gamma(\epsilon) \rvert \lVert \partial_r f \rVert_{\infty} \mathrm{vol}(S^{n-1}) \varepsilon^{n-1} \rightarrow 0,\ \varepsilon \rightarrow 0,
  \end{equation*}
  mientras que
  \begin{equation*}
\int_{r=\varepsilon} f \partial_r \Gamma dS = - \frac{1}{\mathrm{vol}(S^{n-1}) \varepsilon^{n-1}} \int_{r=\varepsilon} f dS \rightarrow f(0),\ \varepsilon\rightarrow 0.
  \end{equation*}
  Concluimos que $(\Gamma * \Delta f) (0)=f(0)$. La igualdad en los demás puntos se sigue de la invariancia bajo traslaciones.

  Para la segunda igualdad, podemos derivar bajo el signo integral y poner
  \begin{equation*}
\Delta (\Gamma * f) = \int_{\RR^n} \Gamma(y) \Delta_xf(x-y) dy = \Gamma * \Delta f = f,
  \end{equation*}
que es lo que queríamos probar.
\end{proof}

\subsection{} Hemos reducido el problema al caso con $\rho=0$. Supongamos entonces que $\phi$ es una solución débil de $\Delta \phi = 0$ en $\Omega$. Se trata de probar que $\phi$ es $C^\infty$ en un abierto $\Omega'$ cuya adherencia está contenida en el interior de $\Omega$. Tomemos un número $\varepsilon >0$ tal que la distancia entre $\Omega'$ y el complementario de $\Omega$ es mayor que $\varepsilon$.

La clave ahora será utilizar la \emph{propiedad del valor medio} de las funciones armónicas. Ésta nos dice que, si $\psi \in C^\infty(\Omega)$ es una función armónica, entonces, para toda bola cerrada $\bar{B}\subset \Omega$ y para todo punto $x\in B$, se tiene que
\begin{equation*}
\psi(x) =\frac{1}{\mathrm{vol}(\partial B)} \int_{\partial B} \psi = \frac{1}{\mathrm{vol}(B)} \int_B \psi.
\end{equation*}

Comenzamos por tomar una función diferenciable $\alpha:\RR\rightarrow \RR$ con $\alpha(r)$ constante para $r$ pequeño, con $\alpha(r)>0$ para $r\geq \varepsilon$ y tal que
\begin{equation*}
\mathrm{vol}(S^{n-1}) \int_0^\infty r^{n-1} \alpha(r) dr = 1.
\end{equation*}
Definimos $A(x)=\alpha(\lVert x \rVert)$, para $x\in \RR^n$. Esta $A$ es $C^\infty$ y cumple que $\int_{\RR^n} A = 1$. Supongamos ahora que $\psi\in C^\infty(B(0,\varepsilon))$ es una función armónica. Usando la propiedad del valor medio y cambiando a coordenadas polares, tenemos que
\begin{equation*}
(A*\psi)(0) = \int_{\RR^n} A(-x) \psi(x) dx = \int_0^\infty \int_{S^{n-1}_r} r^{n-1} \alpha(r) \psi(r,\theta) d\theta dr = \psi(0)
\end{equation*}
Por tanto, por la invariancia por traslaciones, se sigue que, para toda función $\psi \in C^\infty(\RR^n)$ tal que su laplaciano $\Delta\psi$ tiene soporte compacto $K\subset \RR^n$, se tiene que $A*\psi - \psi = 0$ en el subconjunto
\begin{equation*}
\left\{ x \in \RR^n: d(x,K) \geq \varepsilon  \right\}.
\end{equation*}
En particular, si $\phi$ es $C^\infty$ en $\Omega$, entonces $A*\phi = \phi$ en $\Omega'$. Recíprocamente, para toda función $\phi \in L^2$, la convolución $A* \phi \in C^\infty$. En conclusión, probar que $\phi \in C^\infty(\Omega')$ equivale a probar que $A*\phi = \phi$ en $\Omega'$.

\subsection{} Para terminar, queremos probar que si $\phi$ es una solución débil de $\Delta \phi = 0$ en $\Omega$, entonces $A*\phi = \phi$ en $\Omega'$. Para ver esto basta ver que, para toda función $f\in C^\infty(\Omega')$ con soporte compacto, se tiene que
\begin{equation*}
\langle f, \phi - A*\phi \rangle_{L^2} = 0.
\end{equation*}
Ponemos $h=\Gamma* (f- A*f)= \Gamma*f - A* \Gamma* f$. Como $\Gamma*f$ es $C^\infty$ y $\Delta \Gamma * f = f$, tenemos que $\Delta \Gamma * f$ se anula fuera del soporte de $f$ y, por tanto, $A* \Gamma * f = \Gamma * f$ en el subconjunto
\begin{equation*}
\left\{ x \in \RR^n: d(x,\mathrm{sop}(f)) \geq \varepsilon  \right\}.
\end{equation*}
Por tanto, $h$ tiene suporte compacto en $\Omega$ y, como $\phi$ es una solución débil de $\Delta \phi = 0$, tenemos que
\begin{equation*}
\langle \Delta h, \phi \rangle_{L^2} = 0.
\end{equation*}
Pero $\Delta h = \Delta \Gamma * (f - A* f) = f- A*f$, luego
\begin{equation*}
0= \langle \Delta h, \phi \rangle_{L^2} = \langle f-A*f,\phi \rangle_{L^2} = \langle f, \phi-A*\phi \rangle_{L^2},
\end{equation*}
como queríamos probar.
%
%\chapter{Superficies de Riemann compactas}
%a
