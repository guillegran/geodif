...
\begin{corol}[Lema de Hopf]
  Sea $M\subset \RR^N$ una subvariedad regular compacta y orientada y sea $\bm{n}:M\rightarrow S^{N-1}$ la \emph{aplicación de Gauss}, que asocia a cada punto $x\in M$ el \emph{vector unitario normal} $\bm{n}(x)\in S^{N-1}$ en el sentido compatible con la orientación. Se tiene que
  \begin{equation*}
...
\begin{proof}
  Tomamos cualquier vector unitario $\bm{u}\in S^{N-1}\subset \RR^N$ que sea un valor regular de $\bm{n}$ y tomamos su proyección ortogonal a cada espacio tangente $T_xM$, de modo que obtenemos un campo vectorial $X$ en $M$; detalladamente
  \begin{equation*}
...
\ind_x X = \epsilon_{\bm{n}}(x).
  ...
  Concluimos que
  \begin{equation*}
\chi(M)=\ind X =\sum_{x\in \bm{n}^{-1}(\bm{u})} \epsilon_{\bm{n}}(x) + \sum_{x\in \bm{n}^{-1}(-\bm{u})} \epsilon_{\bm{n}}(x) =2 \deg \bm{n},
  \end{equation*}
por el teorema de Poincaré--Hopf.
\end{proof}
